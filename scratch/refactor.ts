import { Block, Project, Statement, SyntaxKind } from "ts-morph";

const project = new Project();
project.addSourceFilesAtPaths("apps/vertaflow-crm/src/**/*.ts");
...
for (const sourceFile of files) {
	...
	const setTenantCalls = calls.filter(
		(c) => c.getExpression().getText() === "setTenantContext",
	);
	...
	for (const call of setTenantCalls) {
		...
		const statement = awaitExpr
			? awaitExpr.getParentIfKind(SyntaxKind.ExpressionStatement)
			: call.getParentIfKind(SyntaxKind.ExpressionStatement);

		if (!statement) {
			console.log(
				`Warning: Could not find statement for setTenantContext in ${sourceFile.getFilePath()}`,
			);
			continue;
		}
		...
		if (
			!block ||
			(!Block.isBlock(block) &&
				block.getKind() !== SyntaxKind.SourceFile &&
				block.getKind() !== SyntaxKind.CaseBlock)
		) {
			console.log(
				`Warning: Parent is not a block in ${sourceFile.getFilePath()}`,
			);
			continue;
		}
		...
		if (lastStmt && lastStmt.getKind() === SyntaxKind.ReturnStatement) {
			...
			const innerCodeNoReturn = stmtsAfter
				.slice(0, -1)
				.map((s) => s.getText())
				.join("\n")
				.replace(/\bdb\./g, "tx.");
			const retExpr = lastStmt
				.asKind(SyntaxKind.ReturnStatement)
				?.getExpression();
			const retText = retExpr
				? `return ${retExpr.getText().replace(/\bdb\./g, "tx.")};`
				: "return;";
			wrapper = `return withTenantContext(${dbArg}, ${tenantIdArg}, async (tx) => {\n${innerCodeNoReturn}\n${retText}\n});`;
		}
