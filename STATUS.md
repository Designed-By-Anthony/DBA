...
- Added `apps/web/build/prepare-pages-bundle.mjs` to copy the **full** `.open-next` output into `.vercel/output/static`, rename `worker.js` → `_worker.js`, and drop static files over **25 MiB** so Wrangler can resolve OpenNext’s `./cloudflare/*`, `./middleware/*`, and `./server-functions/*` imports next to the Pages worker.
