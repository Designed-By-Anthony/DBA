...
export const auditRoute = new Elysia().post(
	...
	async ({ request, set }) => {
		...
		try {
			const apiKey =
				process.env.GOOGLE_PAGESPEED_API_KEY?.trim() || undefined;
			...
			const accessibilityScore = toPct(
			...
			if (placesData.found && placesData.rating) {
				...
				mapsScore = Math.round(
					(ratingFactor * 0.7 + countFactor * 0.3) * 100,
				);
			}
				...
				if (reportPersisted && (isGmailConfigured() || isResendConfigured())) {
					try {
						...
						if (isResendConfigured()) {
							tasks.push(
								sendTransactionalEmail({ to: email, subject, html: emailHtml })
									.catch((err) =>
										console.error(
								...
								sendViaGmail(email, subject, emailHtml)
									.then(() => {
										if (process.env.NODE_ENV === "development") {
											console.info(
												`Email receipt dispatched for ${reportId}`,
											);
										}
									})
									.catch((err) =>
