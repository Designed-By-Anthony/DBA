...
test("rewrites vertaflow apex hosts to the VertaFlow upstream", () => {
  const request = new Request("https://designedbyanthony.com/pricing?plan=pro", {
    headers: { host: "vertaflow.io" },
  });

  const response = middleware(request);

  assert.equal(response.headers.get("x-middleware-rewrite"), "https://vertaflow-preview.vercel.app/pricing?plan=pro");
});
...
test("rewrites www.vertaflow.io to the same VertaFlow upstream", () => {
  ...
  assert.equal(response.headers.get("x-middleware-rewrite"), "https://vertaflow-preview.vercel.app/blog");
});
