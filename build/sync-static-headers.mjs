/**
 * Injects CSP strings from ./csp.mjs into:
 *
 * 1. `static-headers.json` — header rules consumed by `scripts/static-parity-server.mjs`
 *    (Playwright E2E harness on :5500).
 * 2. `netlify.toml` — Netlify build/runtime settings for Next.js.
 *
 * Production CSP, baseline security headers, and per-path `X-Robots-Tag` are applied
 * by Next.js `headers()` in `next.config.ts` (same `build/csp.mjs` source of truth).
 */
...
// ---------------------------------------------------------------------------
// 2. netlify.toml — Next.js on Netlify
// ---------------------------------------------------------------------------

const netlifyToml = `# Generated by build/sync-static-headers.mjs — edit that script or root build/csp.mjs, then run npm run sync:static-headers
[build]
...
writeFileSync(netlifyTomlPath, netlifyToml, "utf8");

console.log(
	"sync-static-headers: wrote static-headers.json + netlify.toml (HTML CSP blocks for parity; live headers + CSP come from next.config.ts)",
);
