...
import type { PlacesResult, Competitor } from '@/lib/places';
  ...
  return (
    <div className="w-full animate-in fade-in slide-in-from-bottom-8 duration-700">
      <div className="text-center mb-10 border-b border-white/10 pb-8">
        <h2 className="text-3xl font-display font-medium mb-2">Audit Complete</h2>
        <a
          href={data.url}
          target="_blank"
          rel="noreferrer"
        ...
        {reportId ? (
          <p className="mt-3 text-sm text-[rgba(255,255,255,0.55)]">
            Report reference:{' '}
            <span className="font-mono text-[rgba(255,255,255,0.78)]">{reportId}</span>
          </p>
        ) : null}
      </div>
      ...
      {actions.length > 0 ? (
        <div className="bg-[#0d1117] rounded-xl p-6 mb-8 border border-white/10">
          ...
          <ol className="space-y-3 list-decimal list-inside text-[rgba(255,255,255,0.88)]">
            {actions.map((item, i) => (
              <li key={`${item.priority}-${i}`} className="leading-relaxed pl-1">
      ...
      {places?.found && (places.rating != null || places.userRatingCount > 0) ? (
      ...
      {moz?.found ? (
        <div className="bg-[#0d1117] rounded-xl p-6 mb-8 border border-white/10">
          <h3 className="text-xl font-display mb-4">Authority &amp; backlinks</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center text-sm">
            <div>
              <p className="text-slate-400 mb-1">Domain authority</p>
              <p className="font-medium text-lg">{moz.domainAuthority ?? '—'}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium text-lg">{moz.linkingRootDomains ?? '—'}</p>
            </div>
            <div>
      ...
      {index?.found && index.estimatedIndexedPages != null ? (
        <div className="rounded-xl p-4 mb-8 border border-white/10 bg-white/[0.03]">
          ...
          <p className="text-[rgba(255,255,255,0.88)]">
            ~{index.estimatedIndexedPages.toLocaleString()} pages ({index.source})
          </p>
        </div>
      ) : null}
      ...
      {sitewide ? (
        <div className="rounded-xl p-4 mb-8 border border-white/10 bg-white/[0.03] text-sm">
          <h3 className="font-semibold text-slate-400 uppercase tracking-wider mb-2">Site crawl signals</h3>
          <ul className="space-y-1 text-[rgba(255,255,255,0.78)]">
            <li>
              robots.txt: {sitewide.robotsTxt.exists ? (sitewide.robotsTxt.allowsCrawlers ? 'allows crawlers' : 'may restrict crawlers') : 'not found'}
            </li>
            <li>
              XML sitemap: {sitewide.sitemap.exists
                ? `${sitewide.sitemap.urlCount.toLocaleString()} URL${sitewide.sitemap.urlCount === 1 ? '' : 's'}`
                : 'not found'}
            </li>
            <li>
              HTTPS / redirects: {sitewide.redirectChain.httpToHttps ? 'HTTP→HTTPS' : 'check mixed content'}
              {sitewide.redirectChain.chainLength > 1 ? ` · ${sitewide.redirectChain.chainLength} hops` : ''}
            </li>
          </ul>
        </div>
      ) : null}
        ...
        <div className="bg-[#0d1117] rounded-xl p-6 mb-8 border border-white/10">
          ...
          <ul className="space-y-2 text-sm text-[rgba(255,255,255,0.85)]">
            {data.competitors.slice(0, 4).map((c, i) => (
              <li key={i} className="flex flex-col sm:flex-row sm:items-baseline sm:gap-2 border-b border-white/5 pb-2 last:border-0">
