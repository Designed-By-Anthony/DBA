...
const RECOMMENDED_LOCAL_SCHEMA = ['LocalBusiness', 'Organization', 'WebSite', 'BreadcrumbList', 'FAQ'];
...
function computeReadability(text: string): { score: number | null; level: 'easy' | 'moderate' | 'difficult' | null } {
...
export async function scanHtml(url: string, timeoutMs = 10000): Promise<HtmlScanResult> {
  ...
  try {
    ...
    result.metaDescription = ($('meta[name="description"]').attr('content') || '').trim().substring(0, 300);
    ...
    // ── Open Graph ──
    result.ogTitle = ($('meta[property="og:title"]').attr('content') || '').trim().substring(0, 200);
    result.ogDescription = ($('meta[property="og:description"]').attr('content') || '').trim().substring(0, 300);
    result.ogImage = ($('meta[property="og:image"]').attr('content') || '').trim().substring(0, 500);
    ...
    // ── Social profiles ──
    result.hasSocialLinks = $('a[href*="facebook.com"], a[href*="instagram.com"], a[href*="linkedin.com/company/"]').length > 0;
    ...
    const ctaPatterns = /\b(call|quote|book|contact|schedule|get started|free estimate|request|consult|talk to|reach out|let'?s talk)\b/i;
    let ctaCount = 0;
