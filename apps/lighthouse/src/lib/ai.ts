...
- copywritingAnalysis: Analyze their actual homepage text. Do they have a clear headline? A clear offer? Does the copy speak to a pain point? Is there a clear next step?`;
  ...
  const competitorBlock = params.competitors && params.competitors.length > 0
  ...
  const backlinkBlock = params.domainAuthority != null
...
- Total External Backlinks: ${params.externalBacklinks ?? 'N/A'}`
  ...
  const indexBlock = params.estimatedIndexedPages != null
    ? `\nIndex Coverage:
...
${params.bodyText}`;
  ...
  for (let attempt = 0; attempt < modelsToTry.length; attempt++) {
    ...
    try {
      ...
      if (isFallback) console.info(`Gemini: primary model unavailable, succeeded with ${model}`);
      ...
      return {
        ...
        prioritizedActions: Array.isArray(parsed.prioritizedActions) ? parsed.prioritizedActions.slice(0, 5) : [],
        copywritingAnalysis: parsed.copywritingAnalysis || '',
      };
    } catch (err) {
      const label = isFallback ? 'fallback' : isRetry ? 'retry' : 'primary';
      console.warn(`Gemini ${label} (${model}) failed:`, err instanceof Error ? err.message : err);

      if (!isRetryableError(err) && !isFallback) {
        // Non-transient error (bad JSON, auth failure) — skip straight to deterministic fallback
        console.error('Gemini non-retryable error, using deterministic fallback');
        return null;
      }
    }
  }
...
export function buildFallbackInsight(params: {
  ...
  if (performanceScore >= 90 && seoScore >= 90) {
    parts.push(`Your technical scores are strong &mdash; ${performanceScore} on performance and ${seoScore} on SEO. That&rsquo;s better than most local businesses I audit.`);
  } else if (performanceScore < 50) {
    parts.push(`The first thing that stands out is load speed. Your main visual element takes ${lcp} to appear &mdash; many visitors will leave before that finishes loading.`);
  } else {
    parts.push(`Your site scored ${performanceScore} on performance, ${accessibilityScore} on accessibility, and ${seoScore} on SEO. There&rsquo;s room to tighten things up.`);
  }

  if (params.domainAuthority != null && params.domainAuthority < 10) {
    parts.push(`Your Domain Authority is ${params.domainAuthority}/100 with ${params.linkingRootDomains ?? 'very few'} sites linking to you. Building quality backlinks would be the single biggest lever for organic visibility.`);
  }

  if (params.spamScore != null && params.spamScore > 30) {
    parts.push(`Your spam score came back at ${params.spamScore}/100, which is elevated. This typically means low-quality or irrelevant sites are linking to you, and it can quietly suppress your rankings.`);
  }

  if (params.readabilityScore != null && params.readabilityScore < 45) {
    parts.push(`The copy reads at a difficulty level that may lose some visitors &mdash; simpler, more direct language tends to convert better for service businesses.`);
  }
  ...
  if (seoScore >= 90) strengths.push(`${seoScore}/100 SEO score — Google can read your site clearly`);
  if (accessibilityScore >= 90) strengths.push(`${accessibilityScore}/100 accessibility — inclusive and well-built`);
  if (params.hasLocalBusinessSchema) strengths.push('LocalBusiness schema markup in place for local search');
  if (params.hasTelLink) strengths.push('Click-to-call link present for mobile visitors');
  if (params.sitemapExists) strengths.push('XML sitemap found — helps search engines discover all pages');
  if (params.hasCanonical) strengths.push('Canonical tag set correctly');
  if (params.metaTitleLength && params.metaTitleLength >= 30 && params.metaTitleLength <= 60) strengths.push('Page title is well-optimized for search');
  ...
  if (params.domainAuthority != null && params.domainAuthority < 10) weaknesses.push(`Domain Authority of ${params.domainAuthority}/100 — very low backlink authority`);
  if (params.spamScore != null && params.spamScore > 30) weaknesses.push(`Spam score of ${params.spamScore}/100 — toxic backlinks may be hurting rankings`);
  if (performanceScore < 90) weaknesses.push(`Performance score of ${performanceScore} — page loads slower than recommended`);
  if (seoScore < 90) weaknesses.push(`SEO score of ${seoScore} — missing meta tags or heading structure`);
  if (!params.hasForms && !params.hasTelLink) weaknesses.push('No contact form or phone link — visitors have no easy way to reach you');
  if (params.missingRecommendedSchema?.length) weaknesses.push(`Missing ${params.missingRecommendedSchema.join(' and ')} schema markup`);
  if (params.readabilityScore != null && params.readabilityScore < 45) weaknesses.push('Homepage copy is harder to read than ideal for a general audience');
  if (params.wordCount != null && params.wordCount < 200) weaknesses.push(`Only ${params.wordCount} words on the homepage — too thin for search engines`);
  ...
  if (params.domainAuthority != null && params.domainAuthority < 15) {
    actions.push({ priority: 1, action: 'Start building quality backlinks from local directories, industry associations, and partner businesses to grow Domain Authority', impact: 'high', effort: 'high' });
  }
  if (performanceScore < 90) {
    actions.push({ priority: actions.length + 1, action: `Optimize your largest contentful paint (currently ${lcp}) to under 2.5 seconds — compress images and defer non-critical scripts`, impact: 'high', effort: 'medium' });
  }
  if (!params.hasForms) {
    actions.push({ priority: actions.length + 1, action: 'Add a simple contact form to your homepage so visitors can reach you without picking up the phone', impact: 'high', effort: 'low' });
  }
  if (params.missingRecommendedSchema?.includes('FAQ')) {
    actions.push({ priority: actions.length + 1, action: 'Add FAQ schema to your service pages — this can earn rich snippets in Google search results', impact: 'medium', effort: 'low' });
  }
  if (params.metaDescriptionLength != null && (params.metaDescriptionLength < 70 || params.metaDescriptionLength > 160)) {
    actions.push({ priority: actions.length + 1, action: 'Rewrite your meta description to 120-155 characters with a clear value proposition and call to action', impact: 'medium', effort: 'low' });
  }
  if (actions.length === 0) {
    actions.push({ priority: 1, action: 'Optimize largest contentful paint to under 2.5 seconds', impact: 'high', effort: 'medium' });
    actions.push({ priority: 2, action: 'Review and improve meta description for better click-through from search results', impact: 'medium', effort: 'low' });
    actions.push({ priority: 3, action: 'Ensure a clear call-to-action is visible above the fold on every page', impact: 'high', effort: 'low' });
  }
  ...
  return {
    ...
    copywritingAnalysis: '<p>The detailed copywriting analysis requires our AI reviewer, which was temporarily unavailable during this audit run. The scores and technical findings above are fully accurate. For a deep dive into your homepage messaging, request a re-run or reply to schedule a walkthrough.</p>',
  };
}
