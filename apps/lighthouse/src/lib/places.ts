 ...
 */
export async function scanPlaces(companyName: string, location: string, timeoutMs: number = 8000): Promise<PlacesResult> {
  ...
  try {
    ...
    const res = await fetchWithTimeout(
      url,
      {
        ...
        headers: {
          ...
          'X-Goog-FieldMask': 'places.rating,places.userRatingCount,places.businessStatus,places.primaryType',
        },
        body: JSON.stringify({
          textQuery: query,
        }),
    ...
    console.warn('Places query failed:', err instanceof Error ? err.message : err);
    return result;
  }
}
...
const COMPETITOR_TYPE_WHITELIST = new Set([
  'plumber', 'electrician', 'hvac_contractor', 'roofing_contractor',
  'general_contractor', 'painter', 'landscaper', 'moving_company',
  'locksmith', 'pest_control_service', 'cleaning_service', 'handyman',
  'auto_repair', 'auto_body_shop', 'car_dealer', 'car_wash',
  'restaurant', 'cafe', 'bakery', 'bar', 'pizza_restaurant',
  'meal_delivery', 'meal_takeaway',
  'dentist', 'doctor', 'veterinary_care', 'physiotherapist',
  'hair_salon', 'beauty_salon', 'spa', 'gym', 'yoga_studio',
  'real_estate_agency', 'insurance_agency', 'accounting',
  'lawyer', 'law_firm', 'notary_public',
  'florist', 'pet_store', 'jewelry_store', 'furniture_store',
  'web_designer', 'marketing_agency', 'graphic_designer',
  'photographer', 'videographer',
  'travel_agency', 'hotel', 'lodging',
]);
...
export async function scanCompetitors(
  ...
  if (!primaryType || primaryType === 'establishment' || !COMPETITOR_TYPE_WHITELIST.has(primaryType)) {
    console.info(`Skipping competitor scan: primaryType "${primaryType}" is not in whitelist.`);
    return [];
  }
  ...
  try {
    ...
    for (const place of data.places) {
      ...
      if (placeName.toLowerCase().includes(normalizedCompany) || normalizedCompany.includes(placeName.toLowerCase())) {
        continue;
      }
    ...
    console.warn('Competitor scan failed:', err instanceof Error ? err.message : err);
    return [];
  }
}
