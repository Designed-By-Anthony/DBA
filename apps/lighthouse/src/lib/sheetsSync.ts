...
export interface ProjectSheetData {
  ...
  strengths?: string[];
  weaknesses?: string[];
  prioritizedActions?: Array<string | { priority?: number; action: string; impact?: string; effort?: string }>;
}

export async function createProjectSheet(data: ProjectSheetData): Promise<string | null> {
  try {
    const auth = new google.auth.GoogleAuth({
      scopes: [
    ...
    if (isAudit) {
      ...
      profileRows.push(
        ['', '', '', ''],
        ['AUDIT SCORES', '', '', ''],
        ['Local Trust Score', String(data.trustScore ?? 'N/A'), 'Scores', '/100'],
        ['Performance', String(data.performance ?? 'N/A'), 'Scores', '/100'],
        ['Accessibility', String(data.accessibility ?? 'N/A'), 'Scores', '/100'],
        ['Best Practices', String(data.bestPractices ?? 'N/A'), 'Scores', '/100'],
        ['SEO', String(data.seo ?? 'N/A'), 'Scores', '/100'],
        ['Conversion Score', String(data.conversion ?? 'N/A'), 'Scores', '/100'],
        ['Google Rating', data.rating != null ? `${data.rating}/5.0` : 'Not found', 'Reputation', `${data.reviewCount ?? 0} reviews`],
        ['', '', '', ''],
        ['CORE WEB VITALS', '', '', ''],
        ...
        ['Failed Audits', String(data.failedAuditCount ?? 0), 'Diagnostics', ''],
      ...
      if (data.strengths && data.strengths.length > 0) {
        ...
        data.strengths.forEach((s, i) => profileRows.push([`${i + 1}`, s, 'Strengths', '']));
      }

      if (data.weaknesses && data.weaknesses.length > 0) {
        profileRows.push(['', '', '', '']);
        profileRows.push(['WEAKNESSES', '', '', '']);
        data.weaknesses.forEach((w, i) => profileRows.push([`${i + 1}`, w, 'Weaknesses', '']));
      }
      ...
      if (data.prioritizedActions && data.prioritizedActions.length > 0) {
        ...
        data.prioritizedActions.forEach((a, i) => {
          ...
          profileRows.push([`${i + 1}`, label, 'Actions', typeof a === 'object' ? `Impact: ${a.impact || 'N/A'} | Effort: ${a.effort || 'N/A'}` : '']);
        });
      }
    }
    ...
    const notesRows: string[][] = [
      ['DATE', 'NOTE', 'STATUS'],
      [now, `${isAudit ? 'Audit' : 'Contact'} form submitted. Awaiting follow-up.`, 'New Lead'],
    ];
    ...
    if (folderId) {
      try {
        ...
        console.warn('Could not move sheet to projects folder:', moveErr instanceof Error ? moveErr.message : moveErr);
      }
    }
    ...
    console.error('Project sheet creation failed:', err instanceof Error ? err.message : err);
    return null;
  }
}
