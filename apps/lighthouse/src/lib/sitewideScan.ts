...
function parseRobotsTxt(content: string): Pick<RobotsTxtResult, 'disallowedPaths' | 'allowsCrawlers' | 'sitemapUrls'> {
...
async function scanRobotsTxt(baseUrl: string): Promise<RobotsTxtResult> {
  try {
    ...
    const res = await fetchWithTimeout(`${origin}/robots.txt`, {
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; DBA-Audit-Bot/1.0)' },
    }, 8000);
    ...
    console.warn('robots.txt scan failed:', err instanceof Error ? err.message : err);
...
async function fetchSitemapText(sitemapUrl: string): Promise<string | null> {
  try {
    const res = await fetchWithTimeout(sitemapUrl, {
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; DBA-Audit-Bot/1.0)', Accept: 'application/xml, text/xml' },
    }, 8000);
...
async function scanRedirectChain(url: string): Promise<RedirectChainResult> {
  ...
  try {
    ...
    console.warn('Redirect chain scan failed:', err instanceof Error ? err.message : err);
  }

  const finalUrl = hops.length > 0 ? hops[hops.length - 1].url : url;

  const hasMixedContent = hops.some(
    (h, i) => i > 0 && h.url.startsWith('http://') && hops[i - 1].url.startsWith('https://')
  );
  const httpToHttps = hops.length >= 2 && hops[0].url.startsWith('http://') &&
    hops.some((h) => h.url.startsWith('https://'));

  const firstHost = (() => { try { return new URL(hops[0]?.url || url).hostname; } catch { return ''; } })();
  const lastHost = (() => { try { return new URL(finalUrl).hostname; } catch { return ''; } })();
  const wwwRedirect =
  ...
  try {
    ...
    if (parsed.protocol === 'https:') {
      ...
      const httpRes = await fetchWithTimeout(httpVariant, {
        redirect: 'manual',
        headers: { 'User-Agent': 'Mozilla/5.0 (compatible; DBA-Audit-Bot/1.0)' },
      }, 5000);
