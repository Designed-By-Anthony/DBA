...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [psiResponseRaw, htmlResponseRaw, placesResponseRaw, sitewideResponseRaw, mozResponseRaw] = await Promise.allSettled([
      psiPromise,
      scanHtml(url),
    ...
    if (!psiResponse.ok) {
      ...
      return NextResponse.json(
        { error: hint },
        { status: psiResponse.status === 400 ? 400 : 502, headers: responseHeaders }
      );
    }

    const defaultHtml: HtmlScanResult = { hasLocalBusinessSchema: false, hasTelLink: false, hasSocialLinks: false, bodyText: '', metaTitle: '', metaDescription: '', h1Count: 0, h1Text: '', imgCount: 0, imgsMissingAlt: 0, hasHttps: false, hasForms: false, ctaCount: 0, externalLinkCount: 0, wordCount: 0, h2Count: 0, h3Count: 0, h4PlusCount: 0, headingHierarchyValid: true, canonicalUrl: '', hasCanonical: false, ogTitle: '', ogDescription: '', ogImage: '', twitterCard: '', robotsMeta: '', hasNoIndex: false, hasNoFollow: false, viewportMeta: '', hasViewport: false, langAttribute: '', hasFavicon: false, internalLinkCount: 0, metaTitleLength: 0, metaDescriptionLength: 0, ttfbMs: null, mixedContentCount: 0, schemaTypes: [], missingRecommendedSchema: ['LocalBusiness', 'Organization', 'WebSite', 'BreadcrumbList', 'FAQ'], imgsWithLazyLoad: 0, imgsWithDimensions: 0, imgsWithSrcset: 0, imgsModernFormat: 0, readabilityScore: null, readingLevel: null };
    const htmlData: HtmlScanResult = htmlResponseRaw.status === 'fulfilled' ? htmlResponseRaw.value : defaultHtml;
    const placesData: PlacesResult = placesResponseRaw.status === 'fulfilled' ? placesResponseRaw.value : { found: false, rating: null, userRatingCount: 0, businessStatus: null, primaryType: null };

    const defaultSitewide: SitewideScanResult = { robotsTxt: { exists: false, content: '', disallowedPaths: [], allowsCrawlers: true, sitemapUrls: [] }, sitemap: { exists: false, urlCount: 0, sampleUrls: [], parseFailed: false }, redirectChain: { hops: [], finalUrl: url, chainLength: 0, hasMixedContent: false, httpToHttps: false, wwwRedirect: false, httpRedirectsToHttps: false, httpRedirectStatus: null } };
    const sitewideData: SitewideScanResult = sitewideResponseRaw.status === 'fulfilled' ? sitewideResponseRaw.value : defaultSitewide;

    const defaultMoz: MozMetrics = { found: false, domainAuthority: null, pageAuthority: null, spamScore: null, linkingRootDomains: null, externalBacklinks: null, pagesCrawled: null, lastCrawled: null };
    const mozData: MozMetrics = mozResponseRaw.status === 'fulfilled' ? mozResponseRaw.value : defaultMoz;

    // Derive index coverage from sitemap + Moz data (no external API call needed)
    const indexData: IndexCheckResult = estimateIndexCoverage(sitewideData.sitemap, mozData);

    // Phase 2: Run competitor scan (needs primaryType from Places result)
    let competitors: Competitor[] = [];
    try {
      competitors = await scanCompetitors(company, location, placesData.primaryType, 3);
    } catch (compErr) {
    ...
    let psiData: { lighthouseResult?: LighthousePayload; error?: { message?: string } };
    try {
    ...
    if (!lighthouse?.categories) {
      ...
      console.error('PageSpeed response missing lighthouseResult:', msg || psiData);
      return NextResponse.json(
        {
          error: msg?.includes('Quota') || msg?.includes('quota')
    ...
    if (hasLowScore) {
      for (const key in audits) {
        ...
        if (
          ...
          failedAuditCount++;
          if (!criticalIssue && audit.title && !audit.id?.includes('contentful-paint')) {
    ...
    const techScore = Math.round((performanceScore * 0.5) + (accessibilityScore * 0.25) + (seoScore * 0.25));
    ...
    crawlabilityScore = Math.round((crawlChecks.filter(Boolean).length / crawlChecks.length) * 100);
    ...
    after(async () => {
      ...
      if (reportPersisted && isGmailConfigured()) {
        try {
          ...
          console.error('Error building receipt email in audit route:', emailErr);
        }
      }

      // Dispatch to the central Agency OS Webhook (CRM) to capture the Lead fully natively
      const osWebhook = process.env.AGENCY_OS_WEBHOOK_URL || 'https://admin.designedbyanthony.com/api/webhooks/lead';
