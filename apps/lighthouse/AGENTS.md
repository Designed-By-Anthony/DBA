...
**Google Workspace vs Google Cloud APIs:** Leaving Google Cloud hosting does **not** remove Google APIs from Lighthouse. **PageSpeed Insights**, **Places**, and optional **Gemini** use API keys in this app. **Gmail / Sheets / Drive** folder IDs for the audit pipeline use `GMAIL_SERVICE_ACCOUNT_KEY`, `SHEETS_ID`, `DRIVE_PROJECTS_FOLDER_ID` here. **Client contracts, OAuth browser flow, and primary Drive/Docs** for CRM workflows live on **VertaFlow CRM** (`apps/vertaflow-crm/.env.local` — `GOOGLE_CLIENT_*`, `GOOGLE_SERVICE_ACCOUNT_*`, `GOOGLE_DRIVE_*`, etc.), not on marketing.
