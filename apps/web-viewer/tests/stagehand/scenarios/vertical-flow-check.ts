...
async function runVerticalFlowCheck() {
  await runQAScenario("Vertical Flow Check", async (bot) => {
    ...
    console.log(`[QA Bot] Detected Vertical: ${verticalExtraction.detectedVertical} (Confidence: ${verticalExtraction.confidence})`);

    // 2. Instruct the bot to try and change the vertical (if the UI allows it) or verify specific CRM elements
    await bot.act(`Navigate through the CRM sections. Look for a way to add a lead, ticket, or order depending on the vertical.`);
    ...
    console.log(`✅ Vertical Flow Check Passed: UI behaves correctly for ${verticalExtraction.detectedVertical}.`);
