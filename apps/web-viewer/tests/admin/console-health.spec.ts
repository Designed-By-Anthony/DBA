...
test.describe("CRM console health", () => {
  for (const path of pages) {
    test(`${path} has no page errors, 500s, chart sizing errors, or hydration errors`, async ({ page }) => {
      ...
      page.on("console", (message) => {
        ...
        if (/pg-connection-string|SECURITY WARNING: The SSL modes/i.test(text)) return;
      ...
      await expect(page.locator("body")).not.toContainText("width(-1) and height(-1)");
    });
  }
});
