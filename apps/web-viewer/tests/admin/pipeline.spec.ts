...
test.describe('Pipeline (Kanban)', () => {
  ...
  test('renders all expected pipeline stages', async ({ page }) => {
    const stages = ['New Lead', 'Contacted', 'Proposal Sent', 'In Development', 'Launched'];
  ...
  test('Kanban board has multiple columns', async ({ page }) => {
    ...
    const stageCount = await page.getByText(/New Lead|Contacted|Proposal|Development|Launched/).count();
