...
test.describe('🧯 Smoke Tests (Basic Sanity)', () => {
  test('Admin login page renders without 500 error', async ({ page }) => {
    ...
    await expect(page.locator('text=Agency OS').first()).toBeVisible({ timeout: 15000 });
  });

  test('Portal login page renders without 500 error', async ({ page }) => {
    const response = await gotoPage(page, '/portal');
    expect(response?.status()).not.toBe(500);
    await expect(page.getByRole('heading', { name: /Client Portal/i })).toBeVisible({ timeout: 15000 });
  });
  ...
  test.skip('Service worker intercepts offline navigation', async ({ page, context }) => {
    ...
    const isOfflineMode = text?.toLowerCase().includes('offline') || text?.toLowerCase().includes('internet');
