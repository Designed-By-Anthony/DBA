...
import { Search, Loader2, User, FileText, Settings, Activity, ArrowRight, CornerDownLeft } from "lucide-react";
...
export function CommandPalette() {
  ...
  const [results, setResults] = useState<{ id: string; name: string; email: string; company: string }[]>([]);
  ...
  const staticCommands = [
    { label: "New Automation Rule", href: "/admin/automations", icon: Activity },
      ...
      <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]">
        ...
        >
          ...
          <div className="flex items-center px-4 py-3 border-b border-glass-border bg-surface-1">
            ...
            />
            {loading && <Loader2 className="w-4 h-4 text-(--color-brand) animate-spin shrink-0" />}
            <span className="text-[10px] uppercase font-bold tracking-widest text-text-muted bg-surface-2 px-2 py-1 rounded ml-2">ESC</span>
          </div>
                ...
                {filteredCommands.map((cmd) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <cmd.icon size={15} className="text-text-muted group-hover:text-(--color-brand)" />
                      {cmd.label}
                    </div>
              ...
              <div>
                ...
                {results.map((prospect) => (
                  ...
                  >
                    ...
                    <ArrowRight size={14} className="text-text-muted opacity-0 group-hover:opacity-100 transition-opacity shrink-0 ml-4" />
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="bg-surface-1 border-t border-glass-border px-4 py-2 flex items-center justify-between">
             <div className="flex items-center gap-4 text-[10px] text-text-muted">
               <span className="flex items-center gap-1"><kbd className="bg-surface-2 px-1.5 rounded font-mono border border-glass-border">↑↓</kbd> to navigate</span>
               <span className="flex items-center gap-1"><kbd className="bg-surface-2 px-1.5 rounded font-mono border border-glass-border">↵</kbd> to open</span>
             </div>
             <p className="text-[10px] text-(--color-brand) font-semibold tracking-wider">AGENCY OS</p>
          </div>
        </motion.div>
      </div>
    </AnimatePresence>
