...
export default function AiSupportWidget() {
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      setMessages([...updated, { role: "model", content: "Sorry, I'm having trouble connecting. Please try again or email anthony@designedbyanthony.com." }]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div
      ...
      style={{ background: "var(--color-surface-0)", animation: "slideUp 0.3s ease-out" }}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-glass-border)]" style={{ background: "var(--color-surface-1)" }}>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-white font-medium">Hi! I&apos;m your setup specialist.</p>
        ...
        {escalated && (
          <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 text-xs text-amber-300">
            ...
            <span>A support ticket has been created. Our team will follow up shortly.</span>
          </div>
        )}
      </div>

      {/* Input */}
      <div className="p-3 border-t border-[var(--color-glass-border)]" style={{ background: "var(--color-surface-1)" }}>
        <div className="flex items-center gap-2">
          <input
            type="text"
            value={input}
