...
export default function BookingWidget({ 
  ...
  const [selectedService, setSelectedService] = useState<StripeProductDetail | null>(null);
  ...
  const availableTimes = ["09:00 AM", "10:00 AM", "11:30 AM", "01:00 PM", "03:00 PM"];

  const calculateDeposit = (priceCents: number) => {
    if (depositPolicy.type === 'full') return priceCents;
    if (depositPolicy.type === 'percentage') return Math.round(priceCents * (depositPolicy.amount / 100));
  ...
  const handleBookSlot = async () => {
    ...
    const depositCents = calculateDeposit(selectedService.default_price?.unit_amount || 0);
  ...
  return (
    <div className="bg-white dark:bg-[var(--color-surface-1)] border border-[var(--color-glass-border)] rounded-3xl overflow-hidden shadow-2xl">
      <div className="bg-[var(--color-brand)] p-6 text-center text-white">
        ...
        <p className="opacity-80 text-sm mt-1">Select your service and reserve a time slot.</p>
      </div>
      ...
      <div className="p-6 md:p-8 space-y-8">
        ...
        {/* Step 1: Service */}
        <div>
          <h3 className="text-[10px] uppercase tracking-wider text-[var(--color-text-muted)] font-semibold mb-3">1. Select Service</h3>
          <div className="grid gap-3">
            {services.map(s => (
              ...
              >
                ...
                <span className="font-mono">{formatCents(s.default_price?.unit_amount || 0)}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        {selectedService && (
           <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
             <h3 className="text-[10px] uppercase tracking-wider text-[var(--color-text-muted)] font-semibold mb-3">2. Date & Time</h3>
        ...
        {selectedService && selectedDate && selectedTime && (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-4">
            <h3 className="text-[10px] uppercase tracking-wider text-[var(--color-text-muted)] font-semibold mb-3">3. Your Info</h3>
            ...
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 text-xs text-amber-400 mt-4 leading-relaxed">
              <strong>Secure Hold & Cancellation Policy:</strong> To lock this slot, a hold of <strong>{formatCents(calculateDeposit(selectedService.default_price?.unit_amount || 0))}</strong> will be placed on your cardiovascular. This deposit is forfeit if you cancel within {cancellationHours} hours of {selectedDate} at {selectedTime}.
            </div>
            ...
            <button
             ...
             >
               {loading ? 'Authorizing Secure Vault...' : 'Reserve & Authorize Card'}
             </button>
          </div>
        )}
