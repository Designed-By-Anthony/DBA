...
import { getDashboardStats, getProspects, getEmailHistory } from "./actions";
    ...
    <div className="space-y-6">
        ...
        {kpis.map((kpi, i) => {
          ...
          return (
            <Link
              ...
              style={{ "--kpi-accent": kpi.color, boxShadow: "var(--kpi-accent) 0 0 0 -5px inset" } as React.CSSProperties}
            >
              ...
              <p className="text-2xl font-bold text-white group-hover:text-(--kpi-accent) transition-colors">{kpi.value}</p>
              <p className="text-xs text-text-muted mt-1 flex items-center justify-between">
                {kpi.label}
                <span className="opacity-0 group-hover:opacity-100 transition-opacity ml-2 text-[10px]">&rarr;</span>
              </p>
            </Link>
          );
        })}
              ...
              <div key={p.id} className="px-5 py-3 flex items-center gap-3">
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-red-100 font-medium">
                    {p.name} <span className="text-white/50 text-xs ml-1">({p.status})</span>
                  </p>
                  <p className="text-xs text-red-300">
                    High-risk detected based on engagement gaps and/or ticket friction.
                  </p>
                </div>
                <Link
      ...
      <div className="glass-card p-6 animate-fade-up stagger-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-sm font-semibold text-white">Pipeline forecast</h3>
            <p className="text-xs text-text-muted">
              Raw pipeline value vs stage-weighted forecast (probabilities from your pipeline settings).
            </p>
          </div>
        </div>
        <RevenueChart 
          data={[
            ...
            { name: "Mar", value: (stats?.pipelineValue || 0) * 0.1, forecast: (stats?.weightedPipelineValue ?? stats?.forecastedMrr ?? 0) * 0.5 },
            { name: "Apr", value: (stats?.pipelineValue || 0) * 0.4, forecast: (stats?.weightedPipelineValue ?? stats?.forecastedMrr ?? 0) * 0.8 },
            { name: "May", value: stats?.pipelineValue || 0, forecast: stats?.weightedPipelineValue ?? stats?.forecastedMrr ?? 0 },
          ]} 
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card overflow-hidden animate-fade-up stagger-5">
          <div className="px-5 py-4 border-b border-glass-border flex justify-between items-center">
            <h3 className="text-sm font-semibold text-white">Recent Prospects</h3>
            <Link href="/admin/prospects" className="text-xs text-(--color-brand) hover:underline">
              View All →
            </Link>
          </div>
          <div className="divide-y divide-glass-border">
            {recentProspects.length === 0 ? (
              <div className="p-8 text-center">
                ...
                <Link href="/admin/prospects" className="text-xs text-(--color-brand) hover:underline mt-2 inline-block">
                  Add your first prospect →
                </Link>
              </div>
            ) : (
              recentProspects.slice(0, 5).map((p) => (
                <Link href={`/admin/prospects/${p.id}`} key={p.id} className="px-5 py-3 flex items-center gap-3 hover:bg-surface-3 transition-colors cursor-pointer group">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm text-white truncate flex items-center gap-2 group-hover:text-(--color-brand) transition-colors">
                       ...
                       {p.leadScore !== undefined && (
                         <span className="text-[10px] text-(--color-brand) bg-brand-subtle rounded px-1" title="Lead Score">
                           ⭐️ {p.leadScore}
                         </span>
                       )}
                    </p>
                    <p className="text-xs text-text-muted truncate">{p.email || p.company || "No email"}</p>
                  </div>
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                    p.status === "launched" ? "bg-emerald-500/10 text-emerald-400" :
                    p.status === "dev" ? "bg-blue-500/10 text-blue-400" :
                    p.status === "proposal" ? "bg-amber-500/10 text-amber-400" :
                    p.status === "contacted" ? "bg-sky-500/10 text-sky-400" :
        ...
        <div className="glass-card overflow-hidden animate-fade-up stagger-6">
          <div className="px-5 py-4 border-b border-glass-border flex justify-between items-center">
            ...
            <Link href="/admin/email/history" className="text-xs text-(--color-brand) hover:underline">
              View All →
            </Link>
          </div>
          <div className="divide-y divide-glass-border">
            {recentEmails.length === 0 ? (
              <div className="p-8 text-center">
                ...
                <Link href="/admin/email" className="text-xs text-(--color-brand) hover:underline mt-2 inline-block">
                  Compose your first email →
                </Link>
              </div>
            ) : (
              recentEmails.slice(0, 5).map((e) => (
                <div key={e.id} className="px-5 py-3 flex items-center gap-3 hover:bg-surface-3 transition-colors">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm shrink-0 ${
                    e.status === "sent" ? "bg-emerald-500/10 text-emerald-400" :
                    e.status === "scheduled" ? "bg-amber-500/10 text-amber-400" :
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-text-muted truncate">To: {e.prospectName}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-xs text-text-muted">
                      {e.opens || 0} opens · {Array.isArray(e.clicks) ? e.clicks.length : 0} clicks
                    </p>
                  </div>
                </div>
              ))
