...
import { getDb, withTenantContext, fileAttachments } from "@dba/database";
...
const attachmentSchema = z.object({
  entityType: z.enum(["appointment", "lead", "order", "event", "inventory_item", "ticket", "contract"]),
  entityId: z.string(),
  url: z.string().url(),
...
export async function createFileAttachment(data: z.infer<typeof attachmentSchema>) {
...
export async function getBeforeAfterPhotos(appointmentId: string) {
  ...
  return withTenantContext(db, orgId, async (tx) => {
    ...
    return {
      ...
      other: all.filter((f) => !f.fileName.startsWith("BEFORE_") && !f.fileName.startsWith("AFTER_")),
    };
  });
}
