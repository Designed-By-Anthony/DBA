...
export async function createInventoryItem(data: z.infer<typeof createItemSchema>) {
...
export async function updateInventoryItem(
  ...
  return withTenantContext(db, orgId, async (tx) => {
    const [item] = await tx
      ...
      .where(and(eq(inventoryItems.id, itemId), eq(inventoryItems.tenantId, orgId)))
      .returning();
...
export async function listInventoryItems(search?: string): Promise<InventoryItemRow[]> {
  const { orgId } = await auth();
  if (!orgId) throw new Error("Unauthorized");
  const db = requireDb();
...
export async function getInventoryItem(itemId: string): Promise<InventoryItemRow | null> {
  const { orgId } = await auth();
  if (!orgId) throw new Error("Unauthorized");
  const db = requireDb();
