...
import { Mail, MessageSquare, Ticket, Inbox, Reply } from "lucide-react";
...
export default function InboxClient({ initialItems }: { initialItems: InboxItem[] }) {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'ticket': return <Ticket size={14} className="text-amber-400" />;
    ...
    <div className="flex h-[calc(100vh-120px)] border border-glass-border rounded-xl overflow-hidden glass-card mt-6">
      ...
      <div className="w-[350px] border-r border-glass-border bg-surface-1 flex flex-col">
        <div className="p-4 border-b border-glass-border flex items-center justify-between">
          ...
          <span className="text-xs bg-surface-3 px-2 py-1 rounded text-text-muted font-mono">{items.length} records</span>
        </div>
          ...
          {items.map((item) => (
            ...
            >
              <div className="flex justify-between items-start mb-1">
                <p className="text-sm font-medium text-white truncate pr-2">{item.sender}</p>
              ...
              <div className="flex items-center gap-2 mb-1">
                 ...
                 <p className="text-xs text-text-gray truncate font-medium">{item.subject}</p>
              </div>
              <p className="text-xs text-text-muted truncate opacity-80">{item.preview}</p>
              
              {!item.isRead && (
                <div className="mt-2 text-[10px] bg-red-500/20 text-red-300 w-max px-1.5 py-0.5 rounded uppercase font-bold tracking-wider">Unread</div>
              )}
            </button>
          ))}
          {items.length === 0 && (
            <div className="p-8 text-center">
              <Inbox size={36} className="mx-auto mb-3 text-text-gray opacity-30" />
              ...
              <p className="text-xs text-text-muted mb-4 max-w-[240px] mx-auto">
                Emails, tickets, and chat messages will flow here as your clients interact.
              </p>
              <a href="/email" className="text-xs text-(--color-brand) hover:underline">
               ...
               <div className="flex justify-between items-start mb-4">
                 <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-surface-3 flex items-center justify-center">
                   ...
                   <div>
                     <h2 className="text-lg font-semibold text-white">{activeItem.subject}</h2>
                    <p className="text-xs text-text-muted">{activeItem.sender} • {new Date(activeItem.createdAt).toLocaleString()}</p>
                   </div>
                 </div>
                 <div className="flex gap-2 text-[10px] uppercase font-bold tracking-wider">
                  <span className="bg-surface-2 text-white px-2 py-1 rounded">{activeItem.type}</span>
                   <span className="bg-emerald-500/10 text-emerald-400 px-2 py-1 rounded">{activeItem.status}</span>
                 </div>
               </div>
            </div>
            ...
            <div className="p-4 border-t border-glass-border bg-surface-1">
              <form onSubmit={handleReply} className="relative">
                ...
                <textarea
                  ...
                  placeholder={isInternalNote ? "Leave a private note for the team..." : `Reply via ${activeItem.type}...`}
