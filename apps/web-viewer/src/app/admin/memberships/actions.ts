...
import { getDb, withTenantContext, memberships } from "@dba/database";
...
export async function updateMembershipStatus(
  ...
  return withTenantContext(db, orgId, async (tx) => {
    const [updated] = await tx
      ...
      .where(and(eq(memberships.id, membershipId), eq(memberships.tenantId, orgId)))
      .returning();
...
export async function listMemberships(prospectId?: string) {
  ...
  return withTenantContext(db, orgId, async (tx) => {
    ...
    return tx.select().from(memberships).where(and(...conditions)).orderBy(memberships.createdAt);
  });
}
