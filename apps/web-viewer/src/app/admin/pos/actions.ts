...
const orderItemSchema = z.object({
  ...
  modifiers: z.array(z.object({ name: z.string(), priceCents: z.number() })).default([]),
});

const createOrderSchema = z.object({
  orderType: z.enum(["dine_in", "takeout", "delivery", "retail_pos", "ecommerce", "catering"]),
...
export async function createOrder(data: z.infer<typeof createOrderSchema>) {
  ...
  return withTenantContext(db, orgId, async (tx) => {
    ...
    const num = latest ? parseInt(latest.orderNumber.replace("ORD-", ""), 10) + 1 : 1;
...
export async function updateOrderStatus(
  orderId: string,
  status: "new" | "preparing" | "ready" | "completed" | "cancelled" | "refunded"
) {
...
export async function listOrders(
  ...
  return withTenantContext(db, orgId, async (tx) => {
    ...
    if (statusFilter) {
      conditions.push(
        eq(orders.status, statusFilter as "new" | "preparing" | "ready" | "completed" | "cancelled" | "refunded")
      );
    }
