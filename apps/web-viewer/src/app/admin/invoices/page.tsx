...
import {
  Receipt, Send, Eye, CheckCircle, Clock, AlertCircle,
  Search, RefreshCw, DollarSign, XCircle,
} from "lucide-react";
...
export default function InvoicesPage() {
  ...
  const filtered = invoices.filter((inv) => {
    ...
    return inv.invoiceNumber.toLowerCase().includes(q) || (inv.prospectId?.toLowerCase().includes(q) ?? false);
  });

  const totalRevenue = invoices.filter((i) => i.status === "paid").reduce((s, i) => s + i.totalCents, 0);
  const totalOutstanding = invoices.filter((i) => i.status === "sent" || i.status === "overdue").reduce((s, i) => s + i.totalCents, 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Revenue Collected", value: formatCents(totalRevenue), color: "#10b981" },
          { label: "Outstanding", value: formatCents(totalOutstanding), color: "#f59e0b" },
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <button onClick={loadInvoices} className="p-2 rounded-lg border border-glass-border text-text-muted hover:text-white hover:bg-surface-2 transition-colors">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-glass-border bg-surface-1/50">
              <th className="text-left py-3 px-4 text-text-muted font-medium">Invoice #</th>
              ...
              <th className="text-left py-3 px-4 text-text-muted font-medium">Due Date</th>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="py-16 text-center">
                  <Receipt size={40} className="mx-auto text-text-muted/30 mb-3" />
                  <p className="text-text-muted">No invoices yet</p>
                  <p className="text-xs text-text-muted/60 mt-1">Invoices are generated from signed contracts.</p>
                </td>
              </tr>
            ) : (
              filtered.map((inv) => {
                ...
                return (
                  <tr key={inv.id} className="border-b border-glass-border/50 hover:bg-surface-1/30 transition-colors group">
                    <td className="py-3.5 px-4 font-mono text-white font-medium">{inv.invoiceNumber}</td>
                    <td className="py-3.5 px-4 text-text-muted">{inv.prospectId || "—"}</td>
                    ...
                    <td className="py-3.5 px-4 text-right text-white font-medium">{formatCents(inv.totalCents)}</td>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        {inv.status === "draft" && (
                          <button onClick={() => handleSend(inv.id)} className="p-1.5 rounded-lg hover:bg-blue-500/10 text-blue-400" title="Send">
                            <Send size={14} />
                          </button>
                        )}
                        {(inv.status === "sent" || inv.status === "overdue") && (
                          <button onClick={() => handleMarkPaid(inv.id)} className="p-1.5 rounded-lg hover:bg-emerald-500/10 text-emerald-400" title="Mark Paid">
                            <DollarSign size={14} />
                          </button>
                        )}
                        <button onClick={() => toast.info("Invoice detail view coming soon")} className="p-1.5 rounded-lg hover:bg-surface-2 text-text-muted hover:text-white" title="View">
                          <Eye size={14} />
                        </button>
                      </div>
                    </td>
