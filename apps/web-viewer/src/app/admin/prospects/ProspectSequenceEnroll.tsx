...
import { listEmailSequences, enrollProspectInSequence } from "../email/sequences/actions";
import type { EmailSequenceDefinition } from "@/lib/types";

export default function ProspectSequenceEnroll({ prospectId }: { prospectId: string }) {
  ...
  if (sequences.length === 0) {
    return (
      <p className="text-xs text-text-muted">
        ...
        <a href="/admin/email/sequences" className="text-(--color-brand) hover:underline">
          Create one
        </a>
      </p>
    );
