...
import { pipelineStages, dealSources } from "@/lib/theme.config";
...
export default function ProspectsPage() {
  ...
  const [dupHint, setDupHint] = useState<{ id: string; name: string } | null>(null);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ name: "", email: "", phone: "", company: "", website: "", targetUrl: "", dealValue: "", source: "", tags: "", notes: "", status: "lead" });
      setShowAdd(false);
      await loadProspects();
    } catch {
  ...
  return (
    <div className="space-y-4">
      ...
      {showAdd && (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              {dupHint && (
                <p className="text-[11px] text-amber-400 mt-1">
                  Possible duplicate:{" "}
                  <Link href={`/admin/prospects/${dupHint.id}`} className="underline hover:text-amber-300">
            ...
            <div>
              <label className="block text-xs text-text-muted uppercase tracking-wider mb-1.5">Phone</label>
              <input
                value={form.phone}
            ...
            <div>
              <label className="block text-xs text-text-muted uppercase tracking-wider mb-1.5">Company</label>
              <input
                value={form.company}
            ...
            <div>
              <label className="block text-xs text-text-muted uppercase tracking-wider mb-1.5">Deal Value ($)</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-xs text-text-muted uppercase tracking-wider mb-1.5">Source</label>
              <select
                value={form.source}
            ...
            <div>
              <label className="block text-xs text-text-muted uppercase tracking-wider mb-1.5">Stage</label>
              <select
                value={form.status}
            ...
            <div>
              <label className="block text-xs text-text-muted uppercase tracking-wider mb-1.5">Tags (comma-separated)</label>
              <input
                value={form.tags}
          ...
          <div>
            <label className="block text-xs text-text-muted uppercase tracking-wider mb-1.5">Notes</label>
            <textarea
              value={form.notes}
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
        ...
        />
        {selected.size > 0 && (
          <div className="flex gap-2 items-center flex-wrap" style={{ animation: "fade-up 0.15s ease-out" }}>
            <span className="text-xs text-text-muted">{selected.size} selected</span>
            <select
              onChange={(e) => {
                if (e.target.value) handleBulkStatus(e.target.value as ProspectStatus);
                e.target.value = "";
              }}
        ...
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-text-muted">
              {search ? "No prospects match your search" : "No prospects yet — add your first one above!"}
            </p>
          </div>
        ) : (
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-glass-border">
                ...
                <th className="px-4 py-3 text-xs text-text-muted uppercase tracking-wider font-medium">Name</th>
                <th className="px-4 py-3 text-xs text-text-muted uppercase tracking-wider font-medium hidden md:table-cell">Email</th>
                <th className="px-4 py-3 text-xs text-text-muted uppercase tracking-wider font-medium hidden lg:table-cell">Company</th>
                <th className="px-4 py-3 text-xs text-text-muted uppercase tracking-wider font-medium">Status</th>
                <th className="px-4 py-3 text-xs text-text-muted uppercase tracking-wider font-medium hidden lg:table-cell">Value</th>
                <th className="px-4 py-3 text-xs text-text-muted uppercase tracking-wider font-medium hidden xl:table-cell">Source</th>
            ...
            <tbody className="divide-y divide-glass-border">
              {filtered.map((p) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <Link href={`/admin/prospects/${p.id}`} className="text-white font-medium truncate hover:text-(--color-brand) transition-colors block">{p.name}</Link>
                        {p.phone && <p className="text-xs text-text-muted truncate">{p.phone}</p>}
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-text-gray hidden lg:table-cell truncate">{p.company}</td>
                  <td className="px-4 py-3">
                    <select
                      value={p.status}
                      onChange={async (e) => {
                  ...
                  <td className="px-4 py-3 text-xs text-text-muted hidden xl:table-cell">{p.source || "—"}</td>
                  <td className="px-4 py-3">
                    <button
