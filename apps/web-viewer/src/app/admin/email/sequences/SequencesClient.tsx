...
export default function SequencesClient({
  ...
  const remove = async (id: string) => {
    if (!confirm("Delete this sequence? Active enrollments may still process until cancelled.")) return;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-xl border border-glass-border bg-glass-bg p-6 space-y-4">
        ...
        />
        {steps.map((st, i) => (
          <div key={i} className="grid grid-cols-1 md:grid-cols-4 gap-2 border border-glass-border rounded-lg p-3">
            <div>
              <label className="text-[10px] text-text-muted uppercase">Delay (hours)</label>
              <input
                type="number"
                min={0}
                value={st.delayHours}
            ...
            >
              <div>
                ...
                <p className="text-xs text-text-muted">
                  {s.steps?.length || 0} steps · {s.isActive ? "Active" : "Paused"}
                </p>
              </div>
              <div className="flex gap-2">
                <button
