...
export default function KitchenDisplaySystem() {
  const [tickets, setTickets] = useState<KDSTicket[]>([
    {
      ...
      items: [
        { id: '1', name: 'Artisan Burger', qty: 2, mods: ['No Onions', 'Medium Rare'] },
  ...
  const getTicketColor = (createdAt: number) => {
    ...
    if (elapsedMins > 15) return 'border-red-500 bg-red-500/10 shadow-[0_0_15px_rgba(239,68,68,0.2)]';
  ...
  return (
    <div className="h-screen bg-black flex flex-col p-4 overflow-hidden select-none">
      <header className="flex justify-between items-center bg-surface-1 border border-glass-border rounded-xl p-4 mb-4">
        <div>
          <h1 className="text-xl font-bold text-white tracking-widest uppercase">Expediter KDS</h1>
        ...
        <div className="flex gap-4">
          <div className="px-4 py-2 bg-surface-2 rounded-lg border border-glass-border text-sm text-text-muted">
            Network: <span className="text-emerald-400 font-bold ml-1">Online</span>
          </div>
        ...
        {tickets.map(ticket => (
          ...
          >
            ...
            <div className="px-4 py-3 border-b border-glass-border flex justify-between items-center bg-surface-2">
              <div>
                <h3 className="text-2xl font-black text-white">#{ticket.orderNumber}</h3>
                <p className="text-[10px] uppercase font-bold text-text-muted">{ticket.source}</p>
              </div>
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-4 text-white">
              {ticket.items.map((item, idx) => (
                <div key={idx} className="border-b border-glass-border pb-3 last:border-0 cursor-pointer hover:bg-white/5 p-2 rounded-lg transition-colors">
                  ...
                  {item.mods && (
                    <ul className="pl-9 mt-1 space-y-1">
                      {item.mods.map(mod => (
                        <li key={mod} className="text-sm font-bold text-red-400">NO {mod.replace('No ', '').toUpperCase()}</li>
                      ))}
                    </ul>
                  )}
                </div>
