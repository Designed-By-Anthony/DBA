...
import { getDb, withTenantContext, taxRates } from "@dba/database";
...
export async function listTaxRates() {
  ...
  return withTenantContext(db, orgId, async (tx) =>
    tx.select().from(taxRates).where(and(eq(taxRates.tenantId, orgId), eq(taxRates.isActive, true)))
  );
}
...
export async function getDefaultTaxRate(): Promise<number> {
  ...
  return withTenantContext(db, orgId, async (tx) => {
    const [rate] = await tx
      ...
      .from(taxRates)
      .where(and(eq(taxRates.tenantId, orgId), eq(taxRates.isDefault, true), eq(taxRates.isActive, true)))
      .limit(1);
