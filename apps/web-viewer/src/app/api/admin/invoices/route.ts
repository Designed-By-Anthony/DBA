...
import { getDb, invoices, contracts, estimates, withTenantContext } from "@dba/database";
  ...
  try {
    ...
    const result = await withTenantContext(db, orgId, async (tx) => {
      ...
      const nextNum = existing.length > 0
        ? parseInt(existing[0].invoiceNumber.replace(/\D/g, "") || "0", 10) + 1
        : 1;
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const body = z.object({
      id: z.string().uuid(),
      status: z.enum(["draft", "sent", "paid", "overdue", "voided", "refunded"]).optional(),
