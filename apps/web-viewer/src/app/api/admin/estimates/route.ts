...
import { getDb, estimates, withTenantContext } from "@dba/database";
  ...
  try {
    ...
    const result = await withTenantContext(db, orgId, async (tx) => {
      ...
      const nextNum = existing.length > 0
        ? parseInt(existing[0].estimateNumber.replace(/\D/g, "") || "0", 10) + 1
        : 1;
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (body.data.proposalContent !== undefined) updates.proposalContent = body.data.proposalContent;
