 ...
 */
export function validatePublicLead(payload: PublicLeadPayload): ValidationIssue | null {
...
const g = globalThis as unknown as Record<typeof RATE_LIMIT_KEY, RateLimitGlobal | undefined>;
...
export function checkLeadRateLimit(
  ...
  if (!id) return { allowed: true, remaining: opts.limit, retryAfterSeconds: 0 };
  ...
  return { allowed: true, remaining: opts.limit - fresh.length, retryAfterSeconds: 0 };
}
 ...
 */
export function requireTurnstileInProd(env: NodeJS.ProcessEnv = process.env): boolean {
