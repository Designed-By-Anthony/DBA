 ...
 */
export function verifyCronAuth(request: Request): { ok: true } | { ok: false; status: number; message: string } {
  ...
  if (!expected) {
    ...
    if (isProd) {
      console.error("[cron] rejected: CRON_SECRET not configured in production");
