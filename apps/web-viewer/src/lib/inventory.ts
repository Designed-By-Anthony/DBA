...
type DB = { select: any; update: any; insert: any; delete: any; execute: any } & Record<string, any>;
...
export async function checkStock(
  ...
  if (!item) {
    return { available: false, currentStock: 0, isLowStock: false, isOutOfStock: true, stockType: "stock" };
  }

  // non_stock items are always available (services, labor, etc.)
  if (item.stockType === "non_stock") {
    return { available: true, currentStock: 0, isLowStock: false, isOutOfStock: false, stockType: "non_stock" };
  }
