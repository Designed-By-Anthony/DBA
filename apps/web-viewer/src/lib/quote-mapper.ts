...
export function stringFromDocument(data: DocumentData, key: string, fallback = ""): string {
  const value = data[key];
  return typeof value === "string" && value.length > 0 ? value : fallback;
}

function optionalStringFromRecord(data: Record<string, unknown>, key: string): string | undefined {
  const value = data[key];
  return typeof value === "string" && value.length > 0 ? value : undefined;
}

function optionalStringFromDocument(data: DocumentData, key: string): string | undefined {
  const value = data[key];
  return typeof value === "string" && value.length > 0 ? value : undefined;
}
...
function quoteTier(value: unknown): QuoteTier {
  return value === "good" || value === "better" || value === "best" ? value : "standard";
}

function quoteStatus(value: unknown): Quote["status"] {
  return value === "sent" || value === "viewed" || value === "accepted" || value === "declined"
    ? value
    : "draft";
}
...
function quoteItem(value: unknown): QuoteItem | null {
  ...
  if (type === "recurring" && (value.interval === "month" || value.interval === "year")) {
...
export function quoteFromDocument(id: string, data: DocumentData): Quote {
  ...
  const packages = Array.isArray(rawPackages)
    ? rawPackages.map(quotePackage).filter((pkg): pkg is QuotePackage => pkg !== null)
    : [];
