...
const g = globalThis as unknown as Record<typeof globalKey, MailerGlobal | undefined>;
...
function validatePayload(p: SendMailParams): string | null {
  ...
  if (!p.subject || typeof p.subject !== "string") return "`subject` is required";
  ...
  if (tos.some((t) => !t || typeof t !== "string")) return "`to` entries must be strings";
  return null;
}
...
export function findTestEmails(filter: {
  ...
  return getTestOutbox().filter((e) => {
    if (filter.to && !e.to.some((t) => t.toLowerCase().includes(filter.to!.toLowerCase()))) {
      return false;
    }
    if (
