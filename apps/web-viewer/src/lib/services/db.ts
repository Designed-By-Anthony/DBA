...
export async function withCurrentTenant<T>(
  fn: (tx: Parameters<Parameters<TenantContext["db"]["transaction"]>[0]>[0], tenantId: string) => Promise<T>,
): Promise<T> {
...
export async function getTenantDomain(domainId: string): Promise<TenantDomainRow | null> {
  return withCurrentTenant(async (tx, tenantId) => {
    const rows = await tx
      .select()
      .from(tenantDomains)
      .where(and(eq(tenantDomains.tenantId, tenantId), eq(tenantDomains.id, domainId)))
      .limit(1);
