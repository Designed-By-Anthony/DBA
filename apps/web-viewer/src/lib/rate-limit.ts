...
export async function rateLimitAsync(
  ...
  try {
    const { allowed, retryAfter } = await upstashEval(key, Date.now(), windowMs, limit);
...
export function tooManyRequests(retryAfterSeconds: number, headers: HeadersInit = {}): NextResponse {
