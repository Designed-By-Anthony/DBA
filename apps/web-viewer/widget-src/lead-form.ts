...
function scriptParams(): { base: string; tenant: string; sig: string } {
  ...
  if (!tenant || !sig) {
    throw new Error("DBA Lead Widget: add ?tenant=<Clerk org id>&sig=<hmac> to the script URL");
  }
  const base = `${u.origin}`;
...
async function fetchSkin(base: string, tenant: string, sig: string): Promise<Skin> {
  const url = `${base}/api/embed/lead/skin?tenant=${encodeURIComponent(tenant)}&sig=${encodeURIComponent(sig)}`;
  const res = await fetch(url, { method: "GET", credentials: "omit" });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error((err as { error?: string }).error || `Skin request failed (${res.status})`);
  }
...
(function boot() {
  ...
  async function init() {
    ...
    title.appendChild(document.createTextNode(skin.brandName || "Get in touch"));
    root.appendChild(title);

    const form = el("form", { style: { display: "flex", flexDirection: "column", gap: "12px" } });

    const inp = (name: string, label: string, type: string, required: boolean, ph?: string) => {
      const wrap = el("label", { style: { display: "flex", flexDirection: "column", gap: "4px" } });
      wrap.appendChild(
    ...
    const msgWrap = el("label", { style: { display: "flex", flexDirection: "column", gap: "4px" } });
    msgWrap.appendChild(el("span", { style: { fontSize: "12px", color: "#a1a1aa" } }, ["Message"]));
    ...
    const status = el("p", {
      style: { margin: "0", minHeight: "1.2em", fontSize: "13px", color: "#a1a1aa" },
    });
    ...
    const mount = document.getElementById("dba-lead-form") || document.currentScript?.parentElement;
    if (mount) {
