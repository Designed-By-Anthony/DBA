...
async function clearOutbox(
	...
	expect(
		res.ok(),
		`Expected outbox DELETE to succeed (EMAIL_TEST_MODE must be on): ${await res.text()}`,
	).toBeTruthy();
}
...
async function fetchOutbox(
	...
	expect(
		res.ok(),
		`GET /api/test/emails should be enabled under EMAIL_TEST_MODE`,
	).toBeTruthy();
...
test.describe("Email test-fire coverage", () => {
	...
	test("GET /api/test/emails responds 200 under EMAIL_TEST_MODE", async ({
		request,
	}) => {
	...
	test("Lead webhook fires admin notification — captured, not sent", async ({
		request,
	}) => {
		...
		expect(
			res.ok(),
			`lead webhook should accept payload: ${await res.text()}`,
		).toBeTruthy();

		const fired = await fetchOutbox(request, { subject: "New Lead" });
		expect(
			fired.length,
			"Admin notification should have been fired",
	...
	test("Magic-link endpoint fires portal login email when prospect exists", async ({
		request,
	}) => {
		...
		expect(
			testModeLink,
			"magic-link should return testModeLink once prospect exists",
		).toBeTruthy();

		const fired = await fetchOutbox(request, {
			to: email,
	...
	test("Invalid payload returns structured error (no uncaught throw)", async ({
		request,
	}) => {
		const res = await request.post("/api/webhooks/lead", {
		...
		expect(after.length, "No email should fire on invalid lead payload").toBe(
			0,
		);
	});

	test("Outbox isolation — DELETE clears captured emails", async ({
		request,
	}) => {
