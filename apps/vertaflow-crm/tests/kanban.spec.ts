...
test.describe("📋 Kanban Board Tests", () => {
	test("Admin: Moving Kanban column updates prospect status in DB", async ({
		request,
		page,
	}) => {
		...
		const res = await request.post("/api/webhooks/lead", {
			data: {
				name: "Kanban Move Test",
				email: mockEmail,
				secret: "dba-lead-hook-2026",
			},
		});
		...
		const card = page
			.locator("div.p-4.rounded-lg.border", { hasText: "Kanban Move Test" })
			.first();
	...
	test("Admin: Adding quick note saves to prospect timeline", async ({
		request,
		page,
	}) => {
		// 1. Create temporary prospect
		...
		await page.fill(
			'textarea[placeholder="Add a note about this prospect..."]',
			"This is a test E2E note.",
		);
		await page.getByRole("button", { name: "Add Note" }).click();

		// 4. Verify it populates the timeline UI correctly
		await expect(page.getByText("This is a test E2E note.")).toBeVisible({
			timeout: 15000,
		});
	});

	test("Admin: Deleting prospect removes them from the board completely", async ({
		request,
		page,
	}) => {
		// 1. Create temporary prospect
		...
		const card = page
			.locator("div.p-4.rounded-lg.border", { hasText: mockEmail })
			.first();
		await expect(card).toBeVisible({ timeout: 10000 });
		...
		await expect(
			page.locator("div.p-4.rounded-lg.border", { hasText: mockEmail }),
