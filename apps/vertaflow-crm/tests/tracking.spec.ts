...
test.describe("Tracking + compliance endpoints", () => {
	test("open pixel returns a GIF with no-cache headers", async ({
		request,
	}) => {
	...
	test("click tracking rejects an unsigned redirect (open-redirect guard)", async ({
		request,
	}) => {
		const res = await request.get(
	...
	test("click tracking rejects when url missing", async ({ request }) => {
		const res = await request.get("/api/track/click/test-email-123", {
			maxRedirects: 0,
		});
		expect(res.status()).toBe(400);
	});
	...
	test("unsubscribe rejects invalid token", async ({ request }) => {
		const res = await request.get(
			"/api/unsubscribe?id=prospect_123&token=invalid",
		);
		expect(res.status()).toBe(400);
		expect((await res.text()).toLowerCase()).toContain("invalid");
	});
