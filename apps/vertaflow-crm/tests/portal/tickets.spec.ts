...
test.describe("Client Portal — Tickets Page", () => {
	...
	test("lists existing tickets", async ({ page }) => {
		await expect(page.getByText("Logo upload issue")).toBeVisible({
			timeout: 5000,
		});
		await expect(page.getByText("Change the hero headline")).toBeVisible({
			timeout: 5000,
		});
	});
	...
	test("Submit button enables after typing a subject", async ({ page }) => {
		...
		const subjectInput = page
			...
			.or(
				page
					.locator('input[placeholder*="subject"]')
					.or(page.locator('input[placeholder*="help"]')),
			)
			.first();
	...
	test("submitting a ticket shows success message", async ({ page }) => {
		...
		await expect(page.getByText(/submitted|sent|touch|success/i)).toBeVisible({
			timeout: 5000,
		});
	});
...
test.describe("Client Portal — Tickets (Unauthenticated)", () => {
	test("unauthenticated /portal/tickets redirects to portal login", async ({
		page,
	}) => {
