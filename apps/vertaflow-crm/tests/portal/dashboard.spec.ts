...
test.describe("Client Portal — Dashboard (Authenticated)", () => {
	...
	test("shows welcome message with client first name", async ({ page }) => {
		await expect(page.getByText(/Welcome back, Jane/i)).toBeVisible({
			timeout: 5000,
		});
	});

	test("shows company name in subtitle", async ({ page }) => {
		await expect(page.getByText(/Smith Plumbing/i)).toBeVisible({
			timeout: 5000,
		});
	});
	...
	test("renders milestone list", async ({ page }) => {
		...
		await expect(page.getByText("Website Live!")).toBeVisible({
			timeout: 5000,
		});
	});

	test("shows existing support ticket in list", async ({ page }) => {
		await expect(page.getByText("Question about logo sizing")).toBeVisible({
			timeout: 5000,
		});
	});
	...
	test("shows ticket submit form", async ({ page }) => {
		const input = page
			.locator('input[placeholder*="help"]')
			.or(page.locator('input[placeholder*="What"]'));
		await expect(input.first()).toBeVisible({ timeout: 5000 });
	});

	test("submit ticket button is disabled when subject is empty", async ({
		page,
	}) => {
	...
	test("shows project notes from admin update", async ({ page }) => {
		await expect(page.getByText(/finished the homepage/i)).toBeVisible({
			timeout: 5000,
		});
	});
});
...
test.describe("Client Portal — Dashboard (Unauthenticated)", () => {
	test("unauthenticated /portal/dashboard redirects to portal login", async ({
		page,
	}) => {
