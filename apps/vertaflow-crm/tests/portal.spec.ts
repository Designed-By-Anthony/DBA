...
test.describe("Client Portal — Login Page", () => {
	...
	test("submit button exists", async ({ page }) => {
		await expect(
			page.getByRole("button", {
				name: /send|login|sign in|magic link|continue/i,
			}),
		).toBeVisible();
	});
	...
	test("submit button enables after valid email is entered", async ({
		page,
	}) => {
		const input = page.locator('input[type="email"]');
		const submitBtn = page.getByRole("button", {
	...
	test("shows confirmation state after mocked successful submit", async ({
		page,
	}) => {
		...
		await page
			.getByRole("button", { name: /send|login|sign in|magic link|continue/i })
			.click();
