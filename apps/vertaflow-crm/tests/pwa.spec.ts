...
test.describe("PWA — Manifest & Offline", () => {
	...
	test("manifest display mode is standalone (app-like)", async ({
		request,
	}) => {
		const res = await request.get("/manifest.webmanifest");
		const data = (await res.json()) as Record<string, unknown>;
...
test.describe("PWA — New API Endpoints", () => {
	test("GET /api/portal/tickets returns 401 without session", async ({
		request,
	}) => {
	...
	test("POST /api/portal/push-token returns 401 without session", async ({
		request,
	}) => {
	...
	test("DELETE /api/portal/push-token returns 401 without session", async ({
		request,
	}) => {
	...
	test("PATCH /api/admin/tickets/nonexistent returns 404", async ({
		request,
	}) => {
