...
async function openAddClientModal(page: Page) {
	...
	await expect(page.getByTestId("clients-add-modal")).toBeVisible({
		timeout: 15_000,
	});
}
...
test.describe("My Clients Page", () => {
	...
	test("renders the My Clients page with header", async ({ page }) => {
		...
		await expect(
			page.getByText(/manage your client organizations/i),
		).toBeVisible();
	});
	...
	test("Add Client modal opens and contains vertical picker", async ({
		page,
	}) => {
