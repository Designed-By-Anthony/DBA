...
test.describe("Domain Settings", () => {
	...
	test("registers a tenant domain and records the admin notification", async ({
		page,
		request,
	}) => {
		test.skip(
		...
		await expect(
			page.getByRole("heading", { name: /domain settings/i }),
		).toBeVisible({
			timeout: 20_000,
		});
		await expect(
			page.getByText(/Database not configured|unavailable/i),
		).not.toBeVisible();
		...
		expect(
			payload.notifications.some((item) => item.payload.domainName === domain),
		).toBe(true);
	});
});
