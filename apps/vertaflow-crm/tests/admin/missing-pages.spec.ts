...
test.describe("Admin — Missing page coverage (shell assertions)", () => {
	...
	test("/admin/pricebook shows Stripe Price Book heading", async ({ page }) => {
		...
		await expect(page.locator("text=Application Error")).not.toBeVisible();
		await expect(
			page.getByRole("heading", { name: /price book/i }),
	...
	test("/admin/kds shows KDS heading", async ({ page }) => {
		...
		expect(res?.status()).not.toBe(500);
		await expect(page.locator("text=Application Error")).not.toBeVisible();
		await expect(page.getByText(/kds/i).first()).toBeVisible({
			timeout: 20_000,
		});
	});
	...
	test("/admin/settings shows Agency Settings heading", async ({ page }) => {
		...
		await expect(page.locator("text=Application Error")).not.toBeVisible();
		await expect(
			page.getByRole("heading", { name: /agency settings/i }),
		).toBeVisible({ timeout: 20_000 });
	});

	test("/admin/settings/business shows Business Settings heading", async ({
		page,
	}) => {
		...
		await expect(page.locator("text=Application Error")).not.toBeVisible();
		await expect(
			page.getByRole("heading", { name: /business settings/i }),
		).toBeVisible({ timeout: 20_000 });
	});

	test("/admin/billing/upgrade shows Upgrade Required badge", async ({
		page,
	}) => {
