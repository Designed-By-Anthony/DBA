		...
		messages: [
			{
				id: "msg-2",
				from: "client",
				content: "Please update the heading text.",
				createdAt: new Date().toISOString(),
			},
			{
				id: "msg-3",
				from: "admin",
				content: "Updated — please check the latest version.",
				createdAt: new Date().toISOString(),
			},
		],
...
test.describe("Admin — Tickets", () => {
	...
	test("admin tickets page loads without application error", async ({
		page,
	}) => {
	...
	test("lists both tickets in the sidebar", async ({ page }) => {
		await expect(page.getByText("Logo upload question")).toBeVisible({
			timeout: 5000,
		});
		await expect(page.getByText("Change homepage headline")).toBeVisible({
			timeout: 5000,
		});
	});
	...
	test("clicking a ticket shows the detail panel", async ({ page }) => {
		...
		await expect(page.getByPlaceholder(/reply|type your reply/i)).toBeVisible({
			timeout: 5000,
		});
	});
	...
	test("reply textarea accepts input", async ({ page }) => {
		...
		await textarea.fill(
			"You can upload via the Google Drive link we sent you.",
		);
