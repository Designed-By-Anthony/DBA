...
test.describe("lead spam-guard: validatePublicLead", () => {
	...
	test("rejects pathologically long values", () => {
		expect(
			validatePublicLead({ name: "x".repeat(400), email: "x@y.com" })?.field,
		).toBe("name");
		expect(
			validatePublicLead({
				name: "ok",
...
test.describe("lead spam-guard: checkLeadRateLimit", () => {
	...
	test("allows submissions up to the configured limit", () => {
		for (let i = 0; i < 3; i++) {
			const result = checkLeadRateLimit("1.2.3.4", {
				limit: 3,
				windowMs: 60_000,
			});
...
test.describe("lead spam-guard: requireTurnstileInProd", () => {
	test("true when VERCEL_ENV=production", () => {
		expect(
			requireTurnstileInProd({ VERCEL_ENV: "production" } as NodeJS.ProcessEnv),
		).toBe(true);
	});
	...
	test("false for local dev / preview / CI", () => {
		expect(
			requireTurnstileInProd({ NODE_ENV: "development" } as NodeJS.ProcessEnv),
		).toBe(false);
		expect(
			requireTurnstileInProd({ VERCEL_ENV: "preview" } as NodeJS.ProcessEnv),
		).toBe(false);
