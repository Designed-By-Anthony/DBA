...
test.describe("🔥 Fire Tests (Critical Paths)", () => {
	test("Lead Intake Pipeline: Form submit populates Kanban", async ({
		request,
		page,
	}) => {
		...
		await expect(page.getByText("Automated E2E Lead").first()).toBeVisible({
			timeout: 10000,
		});
	});

	test("Contract Generation Action: Returns valid Google Docs URL", async ({
		request,
		page,
	}) => {
		...
		await expect(
			page.getByText(
				/MSA generated|Could not generate contract|View Contract|Google Doc/i,
			),
		).toBeVisible({ timeout: 120_000 });
	});

	test("Payment Link Generation: Stripe API creates session", async ({
		request,
		page,
	}) => {
		...
		await expect(page.getByText("Payment link created: $1,000")).toBeVisible({
			timeout: 15000,
		});
	});
});
