...
test.describe("🗣️ Comms Tests (Client ↔ Owner)", () => {
	test("Client: Submitting a new ticket triggers creation", async ({
		request,
		page,
	}) => {
		// 1. Create Lead
		...
		expect(
			resAuth.ok(),
			`Lead Webhook failed: ${await resAuth.text()}`,
		).toBeTruthy();
		await resAuth.json();
		...
		expect(
			testModeLink,
			"magic-link should return testModeLink when IS_TEST=true",
		).toBeTruthy();
		...
		await expect(
			page.getByText("Redirecting to your dashboard..."),
		...
		await page.fill(
			...
			"This is a ticket generated continuously.",
		);
		await page.getByRole("button", { name: "Submit Ticket" }).click();

		// 5. Verify ticket appears in list immediately for client
		await expect(page.getByText("E2E Ticket Request")).toBeVisible({
			timeout: 10000,
		});
	...
	test("System: Audit request dispatches email notification", async ({
		request,
		page,
	}) => {
		...
		await expect(page.getByText("Audit Target Contact").first()).toBeVisible({
			timeout: 10000,
		});
	});
});
