...
test.describe("API Health Checks", () => {
	test("GET /api/auth/google-token returns redirect (auth flow works)", async ({
		request,
	}) => {
	...
	test("POST /api/portal/magic-link returns error for missing email", async ({
		request,
	}) => {
	...
	test("POST /api/portal/magic-link handles invalid email gracefully", async ({
		request,
	}) => {
		const res = await request.post("/api/portal/magic-link", {
	...
	test("GET /api/track/open/[id] returns pixel (not 500)", async ({
		request,
	}) => {
	...
	test("GET /api/track/click/[id] responds (not 500)", async ({ request }) => {
		const res = await request.get(
			"/api/track/click/test-id-123?url=https://example.com",
			{
				maxRedirects: 0,
			},
		);
	...
	test("POST /api/webhooks/stripe route exists (not 404)", async ({
		request,
	}) => {
...
test.describe("Page Navigation", () => {
	test("all admin routes load without 404 or Application Error", async ({
		page,
	}) => {
		const routes = [
			"/admin",
