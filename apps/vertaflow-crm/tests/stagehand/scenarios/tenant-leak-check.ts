...
async function runTenantLeakCheck() {
	await runQAScenario("Tenant Leak Check", async (bot) => {
		...
		await bot.act(
			`Navigate to the leads or CRM section. Click 'Add Lead' or 'New Prospect'. Create a new lead with the name '${leadName}' and email 'leaktest@example.com'. Submit the form.`,
		);
		...
		if (!createdExtraction.isVisible) {
			throw new Error(
				"Failed to create the test lead. The initial tenant workflow might be broken.",
			);
		}

		// 4. Log out and switch to Tenant B
		await bot.act(
			`Log out of the current account. Then log back in using the credentials for Tenant B (if prompted, use the testing credentials or tenant switcher). Navigate back to the leads/CRM section.`,
		);
		...
		if (leakExtraction.hasLeakedData) {
			throw new Error(
				`[DATA LEAK DETECTED] Tenant B was able to see Tenant A's lead: ${leadName}`,
			);
		}

		console.log(
			"✅ Tenant Leak Check Passed: Multi-tenant data remains isolated.",
		);
		return true;
	});
}
