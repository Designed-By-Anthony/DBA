 ...
 */
const leadSecret = () =>
	process.env.LEAD_WEBHOOK_SECRET || "dba-lead-hook-2026";
...
test.describe("Lead intake API", () => {
	...
	test("OPTIONS /api/lead returns 204 with CORS headers", async ({
		request,
	}) => {
	...
	test("POST /api/lead creates prospect (no client secret)", async ({
		request,
	}) => {
	...
	test("POST /api/lead accepts portable-pack snake_case marketing fields (offer_type, cta_source, …)", async ({
		request,
	}) => {
		...
		const res = await request.post("/api/lead", {
			data: {
				...
				page_context: "playwright_api",
				lead_source: "Playwright lead-intake.spec.ts",
	...
	test("GET /api/webhooks/lead without secret: secretOk false", async ({
		request,
	}) => {
	...
	test("POST /api/webhooks/lead without secret returns 401", async ({
		request,
	}) => {
	...
	test("POST /api/webhooks/lead ping does not create prospect", async ({
		request,
	}) => {
		const res = await request.post("/api/webhooks/lead", {
