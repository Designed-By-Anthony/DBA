...
test.describe("Agentic CRM - Proactive Intelligence", () => {
	test("Lead Scoring calculates and decays correctly based on activities", async ({
		request,
	}) => {
	...
	test('Anti-Churn Engine throws "Attention Needed" on stale leads', async ({
		page,
	}) => {
		...
		await expect(
			page
				.getByText(/pipeline forecast|weighted forecast|pipeline velocity/i)
				.first(),
		).toBeVisible({
			timeout: 20_000,
		});
		...
		const isAiWidgetVisible = await page.isVisible(
			"text=AI Intelligence: Attention Needed",
		);
