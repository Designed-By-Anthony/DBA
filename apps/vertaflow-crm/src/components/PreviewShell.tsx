			...
			<header className="flex-none h-16 sm:h-20 border-b border-white/5 bg-bg-ink/80 backdrop-blur-md flex items-center justify-between px-4 sm:px-8 z-10 relative">
				...
				<div className="flex flex-col">
					...
					<h1 className="text-lg sm:text-xl font-display font-bold text-text-cream leading-none tracking-tight">
						{clientName}{" "}
						<span className="text-white/20 px-2 font-light">|</span> Designed by
						Anthony
					</h1>
				</div>
				...
				<div className="hidden md:flex items-center gap-1 bg-[#090f18]/60 p-1 rounded-full border border-white/5 shadow-inner">
					<button
						...
						className={clsx(
							...
							device === "desktop"
								? "bg-accent-blue/10 text-[#bfdbfe] shadow-[0_0_15px_rgba(59,130,246,0.15)] border border-accent-blue/20"
								: "text-text-gray hover:text-text-cream border border-transparent",
						)}
					>
						Desktop
					</button>
			...
			<main className="flex-1 overflow-hidden relative flex flex-col items-center justify-center p-0 sm:p-8 bg-bg-deeper/50">
				...
				<div
					className={clsx(
						...
						getWidthClass(),
						device === "desktop"
							? "shadow-none sm:rounded-xl ring-0"
							: "sm:rounded-[2.5rem] shadow-[0_28px_70px_-34px_rgba(0,0,0,0.8)] border-[6px] border-[#1a1c23] m-4 sm:m-0 h-[calc(100%-2rem)] sm:h-full",
					)}
				>
					{device !== "desktop" && (
