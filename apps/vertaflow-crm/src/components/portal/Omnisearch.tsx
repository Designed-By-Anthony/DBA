...
const SYSTEM_FEATURES = [
	...
	{
		label: "Prospects Directory",
		href: "/admin/prospects",
		aliases: ["leads", "clients", "users"],
	},
	{
		label: "Pipeline Board",
		href: "/admin/pipeline",
		aliases: ["kanban", "deals", "stages"],
	},
	{
		label: "Email Campaigns",
		href: "/admin/email",
		aliases: ["blast", "send", "marketing"],
	},
	{
		label: "Client Tickets",
		href: "/admin/tickets",
		aliases: ["support", "help", "issues"],
	},
	{
		label: "Billing & Invoices",
		href: "/admin/billing",
		aliases: ["money", "finance", "pay"],
	},
	{
		label: "Automations",
		href: "/admin/automations",
		aliases: ["rules", "workflow", "triggers"],
	},
	{
		label: "Inbox",
		href: "/admin/inbox",
		aliases: ["messages", "chat", "notifications"],
	},
	{
		label: "Agency Settings",
		href: "/admin/settings",
		aliases: ["team", "profile", "invite", "members"],
	},
];
...
export default function Omnisearch() {
	...
	const filteredFeatures = SYSTEM_FEATURES.filter((f) => {
		...
		return (
			f.label.toLowerCase().includes(q) || f.aliases.some((a) => a.includes(q))
		);
	...
	return (
		<div ref={wrapperRef} className="relative w-full max-w-sm hidden md:block">
			...
			>
				<Search
					size={16}
					className="text-text-gray group-hover:text-(--color-brand) transition-colors"
				/>
				<input
					type="text"
					value={query}
					...
					<div className="max-h-[60vh] overflow-y-auto p-2">
						...
						{loading && (
							<div className="flex items-center justify-center p-4">
								<Loader2
									size={18}
									className="animate-spin text-(--color-brand)"
								/>
							</div>
						)}
								...
								{prospects.map((p) => (
									...
									>
										<div className="flex items-center justify-between w-full">
											...
											<ArrowRight
												size={14}
												className="text-text-gray opacity-0 group-hover:opacity-100 transition-opacity"
											/>
										</div>
										{(p.company || p.email) && (
