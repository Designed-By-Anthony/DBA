...
export default function Sidebar({
	...
	const isActive = (href: string) => {
		...
		if (normalizedHref === "/email/history")
			return normalizedPath === "/email/history";
		if (normalizedHref === "/automations")
			return normalizedPath === "/automations";
	...
	return (
        ...
        `}
			>
				...
				<button
					onClick={onToggle}
					className="h-16 flex w-full items-center px-5 border-b border-glass-border shrink-0 hover:bg-surface-2 transition-colors text-left focus:outline-none"
				>
					{!collapsed && (
							...
							>
								<Icon
									...
									className={`shrink-0 transition-colors duration-200 ${
										active
											? "text-(--color-brand)"
											: "text-text-gray group-hover:text-white"
									}`}
								/>
				...
				<div className="border-t border-glass-border p-3 space-y-1">
					...
					<SignOutButton redirectUrl="/">
						<button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-text-gray hover:text-red-400 hover:bg-red-500/5 transition-colors">
							<LogOut size={18} />
