...
function notifIcon(type: string) {
	switch (type) {
		...
		case "new_lead":
			return <UserPlus size={12} className="text-(--color-brand)" />;
		default:
			return <Bell size={12} className="text-text-gray" />;
	}
}
...
async function fetchNotifications(): Promise<{
	...
	try {
		const res = await fetch("/api/admin/notifications?unread=false", {
			cache: "no-store",
		});
...
export default function TopBar({
	...
	const handleNotifClick = (notif: Notification) => {
		...
		const href =
			notif.actionUrl ||
			(notif.referenceId && notif.referenceType === "lead"
					...
					<div className="absolute top-12 right-12 w-80 bg-[#090A0F] border border-glass-border rounded-xl shadow-[0_0_40px_rgba(0,0,0,0.8)] overflow-hidden animate-fade-in z-50">
						<div className="p-3 border-b flex items-center justify-between border-glass-border bg-surface-2">
							...
							<button
								...
								className="text-[10px] text-(--color-brand) hover:underline uppercase tracking-wide"
							>
								Mark all read
							</button>
						</div>
						<div className="max-h-80 overflow-y-auto w-full">
							{notifications.length === 0 ? (
								<div className="p-6 text-center">
									<Bell
										size={24}
										className="mx-auto mb-2 text-text-gray opacity-40"
									/>
								...
								notifications.map((notif) => (
									...
									>
										<div
											className={`absolute left-0 top-0 bottom-0 w-1 ${notifAccentColor(notif.type)}`}
										/>
										<p className="text-xs font-semibold text-white mb-1 flex items-center gap-1.5 pl-2">
											{notifIcon(notif.type)}
											<span className={notif.isRead ? "opacity-70" : ""}>
												{notif.title}
											</span>
											{!notif.isRead && (
												<span className="ml-auto w-1.5 h-1.5 rounded-full bg-(--color-brand) shrink-0" />
											)}
											{(notif.actionUrl || notif.referenceId) && (
												<ArrowRight
													size={10}
													className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity text-(--color-brand)"
												/>
											)}
										</p>
										<p className="text-[11px] text-text-muted line-clamp-2 pl-2">
											{notif.body}
										</p>
										<p className="text-[9px] text-text-gray mt-1.5 uppercase pl-2">
						...
						<div className="p-2 border-t border-glass-border text-center">
							<button
								onClick={() => {
									setShowNotifs(false);
									router.push(stripAdmin("/admin/settings/notifications"));
								}}
