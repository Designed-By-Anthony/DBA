...
export const proxy = clerkMiddleware(async (auth, req) => {
	...
	// Staff CRM hosts: real admin/login subdomains, plus *.vercel.app (preview / *.vercel.app
	// production URL — Host is never `admin.*` there, so without this `/` would serve the
	// legacy public landing in `app/page.tsx`).
	...
	// upstream host is the CRM deployment (`*.vercel.app`) — these branches are the
	...
	if (isPreviewDomain) {
		...
		if (!url.pathname.startsWith("/preview/")) {
			return NextResponse.rewrite(
				new URL(
					`/preview/${subdomain}${url.pathname === "/" ? "" : url.pathname}`,
					req.url,
				),
			);
		}
