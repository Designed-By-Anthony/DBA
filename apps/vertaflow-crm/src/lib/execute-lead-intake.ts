import { activities, getDb, withTenantContext } from "@dba/database";
 ...
 */
export async function executeLeadIntake(
	fields: LeadIntakeSource,
	...
	if (agencyId) {
		try {
			...
			console.error(
				"[lead-intake] SQL insert failed; falling back to id-gen only:",
				sqlErr,
			);
		}
	}
	...
	if (sqlResult) {
		...
		prospectId = await generateClientId(
			getIdSource(company, name),
			agencyId || null,
		);
		isNew = true;
	}
	...
	const appUrl =
		process.env.NEXT_PUBLIC_APP_URL || "https://admin.vertaflow.io";
	...
	try {
		...
		await sendMail({
			from: `VertaFlow <${complianceConfig.fromEmail}>`,
