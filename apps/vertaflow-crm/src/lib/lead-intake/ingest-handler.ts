...
export function ingestManifest(endpoint: IngestEndpointName) {
	return {
		...
		body: {
			...
			metadata:
				"optional; unknown top-level fields are folded in automatically",
...
async function verifyAuth(
	...
	if (
		ingestExpected &&
		ingestProvided &&
		constantTimeEqual(ingestProvided, ingestExpected)
	) {
		return null;
	}
	...
	if (
		legacyExpected &&
		legacyProvided &&
		constantTimeEqual(legacyProvided, legacyExpected)
	) {
		return null;
	}
...
export async function handleIngestPost(
	...
	const turnstileToken =
		typeof (rawJson as Record<string, unknown>).cfTurnstileResponse === "string"
			...
			: typeof (rawJson as Record<string, unknown>)["cf-turnstile-response"] ===
					"string"
	...
	const agencyId = await resolveLeadAgencyId(
		readTenantId(request, parsed.tenantId),
	);
	if (!agencyId) {
		return NextResponse.json(
			{
