...
export function getStripeClient(): Stripe {
	if (!_stripe) {
		...
		if (!key) {
			throw new Error(
				"STRIPE_SECRET_KEY is not set. Add it to .env.local to enable payment features.",
			);
		}
...
export const stripe = new Proxy({} as Stripe, {
	get(_, prop) {
		return (getStripeClient() as unknown as Record<string, unknown>)[
			prop as string
		];
	},
});
...
export async function createSubscription(params: {
	...
	const emailQ = escapeStripeSearchToken(
		params.prospectEmail.trim().toLowerCase(),
	);
...
export async function createInvoiceCheckout(params: {
	...
	const hasRecurring = params.lineItems.some(
		(i) => i.interval && i.interval !== "one_time",
	);
