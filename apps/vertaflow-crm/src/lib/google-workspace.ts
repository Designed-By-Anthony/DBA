...
export async function generateContract(params: {
	...
	const replacements: Record<string, string> = {
		...
		"{{CLIENT_COMPANY}}":
			params.companyName || params.clientName || fill("Company Name"),
		...
		"{{COMPLETION_PAYMENT}}": params.completionPayment
			? `$${params.completionPayment.toLocaleString()}`
			: fill("Completion Payment Amount"),
		"{{MONTHLY_RETAINER}}": params.monthlyRetainer
			? `$${params.monthlyRetainer.toLocaleString()}`
			: fill("Monthly Retainer Amount"),
		...
		"{{PROJECT_SCOPE}}":
			params.projectScope || fill("Project Scope — Confirm in Writing"),
...
export async function createClientFolder(params: {
	...
	const folderUrl =
		...
		`https://drive.google.com/drive/folders/${mainFolderId}`;
