...
function buildSidebar(
	...
	const base: SidebarItem[] = [
		...
		{
			label: overrides.prospects?.label || "Prospects",
			href: "/admin/prospects",
			icon: overrides.prospects?.icon || Users,
		},
		{
			label: overrides.pipeline?.label || "Pipeline",
			href: "/admin/pipeline",
			icon: overrides.pipeline?.icon || GitBranch,
		},
		{
			label: overrides.email?.label || "Email",
			href: "/admin/email",
			icon: overrides.email?.icon || Mail,
		},
		...
		{
			label: overrides.tickets?.label || "Tickets",
			href: "/admin/tickets",
			icon: overrides.tickets?.icon || Ticket,
		},
...
const VERTICALS: Record<VerticalId, VerticalConfig> = {
	...
	general: {
		...
		terminology: {
			...
			emptyProspect:
				"No prospects yet — add someone you are already talking to, or import from a lead.",
		},
		pipelineStages: [
	...
	food: {
		...
		terminology: {
			...
			emptyProspect:
				"No customers yet. Add your first customer to start tracking orders.",
		},
		pipelineStages: [
	...
	beauty: {
		...
		terminology: {
			...
			emptyProspect:
				"No clients yet. Add your first client to start managing appointments.",
		},
		pipelineStages: [
	...
	realestate: {
		...
		terminology: {
			...
			addProspect: "Add Lead",
			emptyProspect:
				"No leads yet. Add your first lead to start tracking deals.",
		},
		pipelineStages: [
			{ id: "lead", label: "New Lead", color: "#3b82f6", probability: 0.1 },
			{
	...
	creative: {
		...
		emoji: "🎯",
		description:
			"Photographer, consultant, accountant, lawyer, marketing agency",
		terminology: {
			prospect: "Client",
			prospects: "Clients",
			pipeline: "Projects",
			...
			addProspect: "Add Client",
			emptyProspect:
				"No clients yet. Add your first client to start managing projects.",
		},
		pipelineStages: [
			{ id: "lead", label: "Inquiry", color: "#3b82f6", probability: 0.1 },
			{
...
export function getVerticalConfig(
	id: VerticalId | string | undefined,
): VerticalConfig {
