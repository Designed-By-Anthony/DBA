...
import { activities, getDb, leads, withTenantContext } from "@dba/database";
	...
	try {
		...
		return await withTenantContext(db, session.user.agencyId, async (tx) => {
			const rows = await tx
				...
				.orderBy(sql`(metadata->>'scheduledTime')::timestamptz asc`);
			...
			const items: CalendarBooking[] = rows.map(({ activity: a, leadName }) => {
				...
				const eventUrl =
					typeof meta.eventUrl === "string" ? meta.eventUrl : null;
