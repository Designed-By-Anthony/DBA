...
export default function EmailHistoryClient({ initialEmails }: Props) {
		...
		<div className="space-y-4">
			<div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
				...
				<button
					...
					disabled={loading}
					className="text-xs px-3 py-1.5 rounded-lg border border-glass-border bg-surface-2 text-text-gray hover:text-white disabled:opacity-50"
				>
				...
				{loading && emails.length === 0 ? (
					...
					<div className="p-12 text-center">
						...
						<p className="text-xs text-text-muted mb-4 max-w-sm mx-auto">
							Once you send emails to your prospects, you&apos;ll see open
							rates, click tracking, and delivery status here.
						</p>
						<a
							href="/admin/email"
							className="inline-flex items-center gap-2 text-xs text-(--color-brand) hover:underline"
						>
					...
					<div className="divide-y divide-glass-border">
						{filtered.map((email) => (
							<div key={email.id}>
									...
									<div className="text-right shrink-0 hidden sm:block">
										...
										<p className="text-[10px] text-text-muted">
											{email.sentAt
												...
												: email.scheduledAt
													? `Scheduled: ${new Date(email.scheduledAt).toLocaleDateString()}`
													: "Draft"}
										</p>
									</div>
								...
								{expandedId === email.id && (
									<div
										className="px-5 pb-4 border-t border-glass-border bg-surface-1"
									...
									>
										<div className="grid grid-cols-2 sm:grid-cols-4 gap-4 py-3">
											...
											<div>
												...
												<p className="text-xs text-text-gray font-mono truncate">
													{email.resendId || "—"}
												</p>
											</div>
										</div>
											...
											<div className="mt-2">
												...
												<div className="space-y-1">
													{email.clicks.map((click, i) => (
														<div
															key={i}
															className="text-xs text-text-gray flex gap-2"
														>
															<span>
																{new Date(click.clickedAt).toLocaleString()}
															</span>
															<span className="text-(--color-brand) truncate">
																{click.url}
															</span>
														</div>
													))}
