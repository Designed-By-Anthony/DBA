...
export default function EmailComposerPage() {
	...
	const [editorMode, setEditorMode] = useState<"plaintext" | "html">(
		"plaintext",
	);
	...
	const handleSendTest = async () => {
		...
		try {
			const finalBody =
				editorMode === "plaintext" ? body.replace(/\n/g, "<br/>") : body;
			const previewList =
				selectedIds.size > 0
					? prospects.filter((p) => selectedIds.has(p.id))
					: filtered.slice(0, 5);
			const pTarget =
				previewList.find((p) => p.id === previewProspectId) || previewList[0];
		...
		<div className="space-y-4">
			...
			{result && (
				...
				>
					{result.success ? (
						<p>
							✅ Successfully {scheduleMode ? "scheduled" : "sent"}{" "}
							{result.sent} email{result.sent !== 1 ? "s" : ""}!
						</p>
					) : (
						<div>
							<p>
								⚠️ Sent {result.sent} email{result.sent !== 1 ? "s" : ""} with{" "}
						...
						{loading ? (
							...
							<div className="p-8 text-center text-text-muted text-sm">
								{search
									...
									: "No prospects with email addresses"}
							</div>
						) : (
							filtered.map((p) => (
								<label
				...
				<div className="lg:col-span-3 space-y-4">
					...
					<div className="rounded-xl border border-glass-border bg-glass-bg backdrop-blur-sm overflow-hidden">
						...
						<div className="border-b border-glass-border divide-y divide-glass-border text-sm">
							<div className="flex items-center px-4 py-2">
								...
								<div className="flex-1 text-white truncate">
									{selectedIds.size === 0 ? (
										...
										<span className="bg-brand-subtle text-(--color-brand) px-2 py-0.5 rounded text-xs">
							...
							<div className="flex items-center px-4 py-2">
								<span className="w-16 text-text-muted font-semibold">
									From:
								</span>
								<input
									value={fromName}
						...
						<div className="bg-surface-1 border-b border-glass-border px-4 py-2 flex items-center justify-between">
							<div className="flex items-center gap-1">
								<button
									...
									className="w-7 h-7 flex items-center justify-center rounded hover:bg-surface-3 text-text-gray font-serif font-bold transition-colors"
								>
									B
								</button>
								<button
									...
									className="w-7 h-7 flex items-center justify-center rounded hover:bg-surface-3 text-text-gray font-serif italic font-bold transition-colors"
								>
									I
								</button>
								<button
									...
									onClick={() => {
										...
										setBody(
											(b) =>
												b.substring(0, s) +
												`<a href="#">` +
												(b.substring(s, e) || "link") +
												"</a>" +
												b.substring(e),
										);
										setEditorMode("html");
									}}
									className="w-7 h-7 flex items-center justify-center rounded text-sm hover:bg-surface-3 text-text-gray font-medium transition-colors underline"
								>
									U
								</button>
							</div>
							...
							<div className="flex items-center gap-2">
								...
								<button
									...
									className="text-[10px] px-2 py-1 rounded bg-brand-subtle text-(--color-brand) font-mono hover:bg-(--color-brand) hover:text-white transition-colors"
								>{`{{name}}`}</button>
								<button
									type="button"
						...
						<div className="px-4 py-3">
							...
							{showPreview && editorMode === "html" ? (
								(() => {
									...
									const pTarget =
										previewList.find((p) => p.id === previewProspectId) ||
										previewList[0];

									const pName = pTarget ? pTarget.name.split(" ")[0] : "Jane";
									const pCompany = pTarget
										? pTarget.company || pTarget.name
										: "Acme Corp";
									const pTargetUrl = pTarget
										? pTarget.targetUrl || "https://acme.com"
										: "https://acme.com";
										...
										<div className="px-4 py-3">
											...
											<div
												...
												dangerouslySetInnerHTML={{
													__html: sanitizeEmailPreviewHtml(
														previewHtml || "<em>Nothing to preview</em>",
													),
												}}
											/>
										</div>
								...
								<textarea
									...
									placeholder={
										editorMode === "plaintext"
											? "Type your message naturally here..."
							...
							<p className="text-[10px] text-text-muted mt-2">
								✅ Links auto-wrapped for click tracking · Unsubscribe footer +
								physical address auto-appended
							</p>
						</div>
					</div>
