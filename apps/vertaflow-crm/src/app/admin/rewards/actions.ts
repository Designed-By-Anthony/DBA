...
export async function redeemGiftCard(code: string, amountCents: number) {
	...
	return withTenantContext(db, orgId, async (tx) => {
		...
		if (card.currentBalanceCents < amountCents) {
			throw new Error(
				`Insufficient balance. Available: $${(card.currentBalanceCents / 100).toFixed(2)}`,
			);
		}
...
export async function awardLoyaltyPoints(data: {
	...
	return withTenantContext(db, orgId, async (tx) => {
		...
		const [existing] = await tx
			...
			.where(
				and(
					eq(loyaltyPoints.prospectId, data.prospectId),
					eq(loyaltyPoints.tenantId, orgId),
				),
			)
			.limit(1);
		...
		if (existing) {
			const [updated] = await tx
				...
				.where(
					and(
						eq(loyaltyPoints.id, existing.id),
						eq(loyaltyPoints.tenantId, orgId),
					),
				)
				.returning();
...
export async function getProspectLoyaltyBalance(
	prospectId: string,
): Promise<number> {
	const { orgId } = await auth();
	if (!orgId) throw new Error("Unauthorized");
	...
	return withTenantContext(db, orgId, async (tx) => {
		const [result] = await tx
			...
			.where(
				and(
					eq(loyaltyPoints.prospectId, prospectId),
					eq(loyaltyPoints.tenantId, orgId),
				),
			)
			.limit(1);
