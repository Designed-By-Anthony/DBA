...
export default function QuoteBuilderPage() {
	...
	const calculateTotal = (tierItems: StripeProductDetail[]) => {
		const recurring = tierItems
			.filter((i) => i.default_price?.type === "recurring")
			.reduce((sum, i) => sum + (i.default_price?.unit_amount || 0), 0);
		const once = tierItems
			.filter((i) => i.default_price?.type !== "recurring")
			.reduce((sum, i) => sum + (i.default_price?.unit_amount || 0), 0);
	...
	const handleSaveQuote = async () => {
		...
		const packagesToSave: QuoteTier[] = multiTier
			? ["good", "better", "best"]
			: ["standard"];
		...
		try {
			...
			const res = await saveQuoteAction(prospectId, {
	...
	if (loading)
		return <div className="p-8 text-white">Loading framework...</div>;
	if (!prospect)
		return <div className="p-8 text-white">Prospect not found</div>;
			...
			<div className="flex items-center gap-4 border-b border-[var(--color-glass-border)] pb-6">
				...
				<div>
					<div className="flex items-center gap-2">
						...
						{prospect.company && (
							<span className="text-[var(--color-text-muted)] mt-1">
								• {prospect.company}
							</span>
						)}
					</div>
					...
					<div className="space-y-2">
						{stripeProducts.length === 0 && (
							<p className="text-xs text-[var(--color-text-muted)]">
								No products in Stripe Pricebook.
							</p>
						)}
						{stripeProducts.map((p) => (
							...
							>
								<div className="flex justify-between items-start">
									<p className="text-sm text-white font-medium group-hover:text-[var(--color-brand)] transition-colors">
										{p.name}
									</p>
								...
								{p.default_price?.type === "recurring" && (
									<p className="text-[10px] text-[var(--color-brand)] mt-1">
					...
					<div className="bg-[var(--color-surface-1)] border border-[var(--color-glass-border)] rounded-xl p-5 flex items-center justify-between">
						<div>
							...
							<p className="text-sm text-[var(--color-text-muted)]">
								Toggle tiers to offer Good/Better/Best options
							</p>
						</div>
							...
							{tiers[activeTier].items.length === 0 ? (
								...
								<div className="space-y-3">
									{tiers[activeTier].items.map((item, idx) => (
										<div
											key={`${item.id}-${idx}`}
											className="flex items-center justify-between p-4 bg-[var(--color-surface-2)] border border-[var(--color-glass-border)] rounded-lg"
										>
											<div>
											...
											<div className="flex items-center gap-4">
												<div className="text-right">
													...
													<p className="text-[10px] text-[var(--color-brand)]">
														{item.default_price?.type === "recurring"
															? "Recurring"
															: "One-Time"}
													</p>
												</div>
												<button
													onClick={() =>
														handleRemoveItemFromTier(activeTier, item.id)
													}
									...
									<div className="mt-6 border-t border-[var(--color-glass-border)] pt-4 flex justify-end gap-8">
										<div className="text-right">
											...
											<p className="text-lg text-white font-mono font-bold">
												{formatCents(
													calculateTotal(tiers[activeTier].items).once,
												)}
											</p>
										</div>
										<div className="text-right">
											...
											<p className="text-lg text-[var(--color-brand)] font-mono font-bold">
												{formatCents(
													calculateTotal(tiers[activeTier].items).recurring,
												)}
											</p>
										</div>
									</div>
