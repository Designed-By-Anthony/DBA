...
import { centsToDollars, dollarsToCents, formatCents } from "@/lib/currency";
...
function ClientUpdatePanel({
	...
	const [note, setNote] = useState(
		((prospect as unknown as Record<string, unknown>).projectNotes as string) ||
			"",
	);
	const [contractSigned, setContractSigned] = useState(
		!!(prospect as unknown as Record<string, unknown>).contractSigned,
	);
...
function CrmTasksPanel({
	...
	const addTask = async () => {
		...
		const res = await createCrmTask(prospectId, {
			title: title.trim(),
			dueAt: iso,
		});
		if (res.success) {
		...
		<div className="rounded-xl border border-glass-border bg-glass-bg p-5 backdrop-blur-sm">
			...
			{loading ? (
				...
				<p className="text-xs text-text-muted">
					No tasks yet. Add a reminder to stay on top of this deal.
				</p>
			) : (
						...
						return (
							...
							>
								...
								<div className="flex-1 min-w-0">
									<p
										className={`text-sm ${task.completed ? "line-through text-text-muted" : "text-white"}`}
									>
										{task.title}
									</p>
		...
		return (
			<div className="text-center py-20">
				...
				<Link
					...
					className="text-(--color-brand) text-sm hover:underline mt-4 inline-block"
				>
					← Back to Prospects
				</Link>
			</div>
					...
					<div>
						...
						{stageMeta && (
							<p className="text-xs text-text-muted mt-1">
								Pipeline: {stageMeta.label} ·{" "}
								{Math.round(stageMeta.probability * 100)}% weight
							</p>
						)}
					</div>
				</div>
				...
				<div className="lg:col-span-2 space-y-6">
					...
					<div className="rounded-xl border border-glass-border bg-glass-bg p-5 backdrop-blur-sm">
							...
							<div className="mt-4 pt-4 border-t border-glass-border">
								...
								<div className="flex gap-1.5 flex-wrap">
									{prospect.tags.map((tag) => (
										<span
											key={tag}
											className="text-xs px-2.5 py-1 rounded-full bg-brand-subtle text-white"
										>
											{tag}
										</span>
									))}
						...
						<div className="mt-4 pt-4 border-t border-glass-border flex flex-wrap gap-3">
							...
							{prospect.calendlyEventUrl && (
								<a
									href={prospect.calendlyEventUrl}
									target="_blank"
									rel="noopener noreferrer"
						...
						<div className="flex gap-1">
							{pipelineStages.map((stage) => {
								...
								const idx = pipelineStages.findIndex(
									(s) => s.id === prospect.status,
								);
								const stageIdx = pipelineStages.findIndex(
									(s) => s.id === stage.id,
								);
					...
					<div className="rounded-xl border border-glass-border bg-glass-bg p-5 backdrop-blur-sm">
						...
						<p className="text-xs text-text-muted mb-3">
							Enroll this contact in a drip sequence. Due emails run on your
							cron or via Sequences → Run now.
						</p>
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
							<div>
								...
								<select
									...
									onChange={(e) => {
										handleFieldUpdate("customPricing", {
											...prospect.customPricing,
											retainerTier: e.target.value,
										});
									}}
							...
							<div>
								...
								<input
									...
									defaultValue={centsToDollars(
										prospect.customPricing?.downPaymentCents || 0,
									)}
									placeholder="e.g. 500"
									onBlur={(e) => {
					...
					<div className="rounded-xl border border-glass-border bg-glass-bg p-5 backdrop-blur-sm">
						...
						<div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
							...
							<button
								...
								disabled={
									actionLoading === "folder" || !!prospect.driveFolderUrl
								}
							...
							<button
								onClick={() => {
									const amount = prompt(
										...
										String(
											centsToDollars(
												prospect.customPricing?.downPaymentCents || 50000,
											),
										),
									);
									if (amount)
										handleCreatePaymentLink("down_payment", parseFloat(amount));
								}}
							...
							<button
								onClick={() => {
									const amount = prompt(
										...
										String(
											centsToDollars(
												prospect.customPricing?.completionPaymentCents ||
													100000,
											),
										),
									);
									if (amount)
										handleCreatePaymentLink("completion", parseFloat(amount));
								}}
								disabled={actionLoading === "payment"}
						...
						<p className="text-[10px] text-text-muted mt-2">
							...
							<code className="text-text-gray">
								GOOGLE_DOCS_MSA_TEMPLATE_ID
							</code>{" "}
