...
import { getDb, hardwareDevices, withTenantContext } from "@dba/database";
...
export async function listDevices(type?: string) {
	...
	return withTenantContext(db, orgId, async (tx) => {
		const conditions = [
			eq(hardwareDevices.tenantId, orgId),
			eq(hardwareDevices.isActive, true),
		];
...
export async function deactivateDevice(deviceId: string) {
	...
	return withTenantContext(db, orgId, async (tx) => {
		const [updated] = await tx
			...
			.where(
				and(
					eq(hardwareDevices.id, deviceId),
					eq(hardwareDevices.tenantId, orgId),
				),
			)
			.returning();
