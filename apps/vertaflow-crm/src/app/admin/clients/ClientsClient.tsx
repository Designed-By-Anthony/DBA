		...
		try {
			...
			setOrgs((prev) => [
				...prev,
				{
					...
					role: "org:admin",
					branding: {
						brandName: newOrgName.trim(),
						brandColor: "#2563eb",
						brandInitial: newOrgName.charAt(0).toUpperCase(),
						verticalTemplate: newOrgVertical,
					},
				},
			]);
			setNewOrgName("");
		...
		<div className="min-h-screen bg-(--color-surface-0) px-4 sm:px-8 py-6">
			...
			<div className="flex flex-col sm:flex-row gap-4 justify-between items-start mb-8 animate-fade-in">
				<div>
					...
					<p className="text-text-muted text-sm mt-1">
						Manage your client organizations. Each client has their own isolated
						CRM and portal.
					</p>
				</div>
				<button
				...
				>
					<div className="glass-card p-6 w-full max-w-lg animate-scale-in relative shadow-[0_0_80px_rgba(37,99,235,0.08),0_24px_48px_rgba(0,0,0,0.6)]">
						...
						<p className="text-xs text-text-muted mb-6">
							Create a new organization for your client. They&apos;ll get their
							own CRM, pipeline, and client portal.
						</p>
									...
									{ALL_VERTICALS.map((v) => {
										...
										return (
											...
											>
												<VIcon
													size={20}
													className={selected ? "text-(--color-brand)" : ""}
												/>
						...
						<div className="flex items-center gap-3 mb-6">
							...
							<div>
								...
								<p className="text-xs text-text-muted">
									Customize how {editingOrg.name}&apos;s portal looks to their
									clients
								</p>
							</div>
						</div>
						...
						<div className="bg-surface-1 rounded-xl p-6 mb-6 border border-glass-border">
							...
							<div className="flex flex-col items-center gap-3">
								...
								<p className="text-[10px] text-text-muted text-center leading-snug">
									Uses the same flow as the live portal. Only emails that match
									a prospect in this organization receive a link.
								</p>
							</div>
						</div>
						...
						<div className="space-y-4">
							...
							<div>
								...
								<input
									...
									value={brandInitial}
									onChange={(e) =>
										setBrandInitial(e.target.value.slice(0, 2).toUpperCase())
									}
									placeholder="S"
									maxLength={2}
			...
			{orgs.length === 0 ? (
				<div className="glass-card p-12 text-center animate-fade-up">
					<Building2
						size={48}
						className="mx-auto mb-4 text-text-gray opacity-30"
					/>
					<h3 className="text-lg font-semibold text-white mb-2">
						No clients yet
					</h3>
					<p className="text-sm text-text-muted mb-6 max-w-md mx-auto">
						Add your first client to get started. Each client gets their own
						CRM, prospect pipeline, email tools, and a branded client portal.
					</p>
					<button
						type="button"
					...
					{orgs.map((org, i) => (
						...
						>
							...
							<div
								...
								style={{
									background:
										(org.branding as Record<string, string>)?.brandColor ||
										"var(--color-brand)",
								}}
							/>
							<div className="p-5">
								<div className="flex items-start gap-3 mb-4">
									...
									>
										{(org.branding as Record<string, string>)?.brandInitial ||
								...
								{/* Stats */}
								{(() => {
									const vt = getVerticalConfig(
										(org.branding as Record<string, string>)?.verticalTemplate,
									);
									return (
										<>
