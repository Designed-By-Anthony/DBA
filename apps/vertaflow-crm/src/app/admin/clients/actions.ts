...
import { getDb, leads, tenants, withTenantContext } from "@dba/database";
			...
			if (db) {
				...
				try {
					...
					if (tenantRows.length > 0) {
						...
						const crm =
							(t.crmConfig as Record<string, unknown> | undefined) ?? {};
...
> {
	...
	if (session.user.id === "dev") {
		return {
			success: false,
			error: "Sign in with Clerk to create client organizations.",
		};
	}
	...
	try {
		...
		return {
			success: false,
			error:
				"Could not create organization. Verify CLERK_SECRET_KEY matches this deployment and your Clerk plan allows creating organizations.",
		};
	}
}
...
export async function updateOrgBranding(
	...
	if (db) {
		...
		await db.update(tenants).set(payload).where(eq(tenants.clerkOrgId, orgId));
	}
