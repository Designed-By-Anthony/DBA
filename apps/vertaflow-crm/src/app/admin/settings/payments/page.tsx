		...
		<div className="space-y-8 max-w-2xl">
			...
			<div className="rounded-xl border border-glass-border bg-surface-1 overflow-hidden">
				<div className="p-5 border-b border-glass-border flex items-center gap-3">
					<div
						className={`w-10 h-10 rounded-lg flex items-center justify-center ${isActive ? "bg-emerald-500/15 text-emerald-400" : "bg-surface-3 text-text-gray"}`}
					>
						<CreditCard size={20} />
					</div>
					<div className="flex-1">
						...
						<p className="text-[11px] text-text-muted">
							{isActive
								? "Your account is fully set up and receiving payments"
								: isOnboarding
									? "Complete your Stripe onboarding to start accepting payments"
									: isRestricted
										? "Action required — your account has restrictions"
				...
				<div className="p-5 space-y-4">
					{!hasAccount && (
						<div className="text-center py-6">
							<CreditCard
								size={40}
								className="mx-auto mb-4 text-text-gray opacity-40"
							/>
							<h3 className="text-sm font-semibold text-white mb-2">
								Accept Payments Online
							</h3>
							<p className="text-xs text-text-muted mb-6 max-w-sm mx-auto">
								Connect with Stripe to accept credit cards, bank transfers, and
								135+ payment methods. You&apos;ll manage your own Stripe
					...
					{isOnboarding && hasAccount && (
						<div className="flex items-start gap-3 p-4 rounded-lg bg-amber-500/10 border border-amber-500/20">
							...
							<div>
								...
								<p className="text-xs text-text-muted mt-1">
									Finish setting up your Stripe account to start accepting
									payments.
								</p>
								<button
									onClick={handleConnect}
					...
					{isRestricted && (
						<div className="flex items-start gap-3 p-4 rounded-lg bg-red-500/10 border border-red-500/20">
							...
							<div>
								...
								<p className="text-xs text-text-muted mt-1">
									Your Stripe account requires attention. Visit the Stripe
									dashboard to resolve.
								</p>
				...
				<div className="flex items-start gap-3">
					...
					<div>
						...
						<p className="text-[10px] text-text-muted mt-1">
							Stripe handles PCI compliance, fraud detection, and regulatory
							requirements. Your customer payment data never touches our
							servers.
						</p>
					</div>
				</div>
...
function StatusBadge({ status }: { status: string | null }) {
	...
	return (
		<span
			className={`px-2.5 py-1 rounded-full text-[10px] font-semibold ${s.bg} ${s.text}`}
		>
			{s.label}
		</span>
	);
...
function StatusCard({
	...
	return (
		<div
			className={`p-3 rounded-lg border text-center ${enabled ? "bg-emerald-500/5 border-emerald-500/20" : "bg-surface-2 border-glass-border"}`}
		>
			<div
				className={`mx-auto mb-1.5 ${enabled ? "text-emerald-400" : "text-text-gray"}`}
			>
				{icon}
			</div>
			<p className="text-[10px] font-medium text-white">{label}</p>
			<p
				className={`text-[9px] mt-0.5 ${enabled ? "text-emerald-400" : "text-text-gray"}`}
			>
