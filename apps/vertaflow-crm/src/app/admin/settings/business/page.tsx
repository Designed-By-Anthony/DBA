		...
		<div className="max-w-4xl mx-auto space-y-8 p-6">
			<div>
				...
				<p className="text-text-muted mt-2">
					Configure platform-wide rules, billing policies, and regulatory
			...
			<div className="grid gap-6 md:grid-cols-2">
				...
				<div className="bg-surface-1 border border-glass-border rounded-2xl p-6 space-y-6">
					<h2 className="text-lg font-bold text-white border-b border-glass-border pb-3">
						Booking Rules & Revenue Protection
					</h2>

					<div className="space-y-3">
						<label className="text-sm font-medium text-text-gray">
							Upfront Deposit Policy
						</label>
						<select
							...
							onChange={(e) =>
								setSettings({
									...settings,
									depositPolicy: e.target
										.value as BusinessSettings["depositPolicy"],
								})
							}
					...
					{settings.depositPolicy !== "full" && (
						<div className="space-y-3">
							<label className="text-sm font-medium text-text-gray">
								{settings.depositPolicy === "percentage"
									? "Deposit Percentage (%)"
									: "Flat Deposit Amount ($)"}
							</label>
							<input
								...
								onChange={(e) => {
									...
									if (settings.depositPolicy === "flat")
					...
					<div className="space-y-3">
						...
						<p className="text-xs text-text-muted mb-2">
							Deposits are forfeit if cancelled within this window.
						</p>
						<input
							type="number"
							...
							onChange={(e) =>
								setSettings({
									...settings,
									cancellationWindowHours: parseInt(e.target.value) || 24,
								})
							}
							className="w-full bg-surface-2 border border-glass-border rounded-lg px-4 py-3 text-sm text-white outline-none focus:border-(--color-brand)"
						/>
				...
				<div className="bg-surface-1 border border-glass-border rounded-2xl p-6 space-y-6">
					...
					<label className="flex items-start gap-4 p-4 rounded-xl border border-glass-border bg-surface-2 cursor-pointer hover:border-(--color-brand) transition-colors">
						<input
							...
							onChange={(e) =>
								setSettings({
									...settings,
									requireDigitalWaiver: e.target.checked,
								})
							}
						...
						<div>
							<p className="text-sm font-medium text-white">
								Require Digital Waivers
							</p>
							<p className="text-xs text-text-muted mt-1">
								Block fitness/studio check-ins if no HTML signature is on file.
							</p>
						</div>
					</label>
					...
					<label className="flex items-start gap-4 p-4 rounded-xl border border-glass-border bg-surface-2 cursor-pointer hover:border-(--color-brand) transition-colors">
						...
						<div>
							...
							<p className="text-xs text-text-muted mt-1">
								Enable digital ordering menus linked directly to KDS systems.
							</p>
						</div>
					</label>
							...
							onChange={(e) =>
								setSettings({
									...
									inventoryMode: e.target
										.value as BusinessSettings["inventoryMode"],
								})
							}
							className="w-full bg-surface-2 border border-glass-border rounded-lg px-4 py-3 text-sm text-white outline-none focus:border-(--color-brand)"
						>
