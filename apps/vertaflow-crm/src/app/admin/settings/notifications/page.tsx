...
import { Bell, Loader2, Lock, Mail, Shield, Smartphone } from "lucide-react";
	...
	[
		{
			...
			description: "When a new prospect enters the pipeline",
		},
		{
			type: "ticket_created",
			label: "Support ticket opened",
			description: "When a client creates a new ticket",
		},
		{
			type: "ticket_reply",
			label: "Ticket reply",
			description: "When a client replies to a ticket",
		},
		{
		...
		},
		{
			...
			description: "When a deal moves stages",
		},
		{
			type: "daily_digest",
			label: "Daily digest",
			description: "Morning summary of pipeline activity",
		},
	];
		...
		try {
			...
			if (res.ok) {
				...
				const personalMap = new Map<
					string,
					{ emailEnabled: boolean; pushEnabled: boolean; inAppEnabled: boolean }
				>();
				...
				<p className="text-xs text-text-muted mt-1">
					...
					{user?.organizationMemberships?.[0]?.role === "org:admin" && (
						<span className="text-(--color-brand)">
							{" "}
							As admin, you can enforce mandatory notifications.
						</span>
					)}
				</p>
			</div>
				...
				<div className="flex items-start gap-3">
					...
					<div className="flex-1">
						...
						<p className="text-[11px] text-text-muted mt-0.5">
							Receive real-time notifications even when Agency OS isn&apos;t
							open
						</p>
					</div>
					...
					>
						...
						<div className="flex justify-center">
							<button
								...
								title={
									event.mandatory
										? "Members cannot disable this event"
										: "Click to enforce for all members"
								}
							>
								<span
									className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all ${event.mandatory ? "left-3.5" : "left-0.5"}`}
								/>
							</button>
						</div>
					</div>
				...
				<div className="flex items-start gap-3">
					...
					<div>
						<p className="text-xs font-medium text-white">
							Admin-Enforced Notifications
						</p>
						<p className="text-[10px] text-text-muted mt-1">
							Events marked as &ldquo;Required&rdquo; cannot be disabled by team
							members. Use this to ensure critical events like new leads and
							payments always reach your team through all channels.
						</p>
					</div>
				</div>
			</div>
...
function ChannelToggle({
	...
	return (
		...
		>
			<span
				className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all ${enabled ? "left-3.5" : "left-0.5"}`}
			/>
		</button>
	);
}
