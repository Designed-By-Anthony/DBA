						...
						<table className="w-full min-w-[720px] text-left text-xs">
							...
							<tbody className="divide-y divide-glass-border">
								{group.records.map((record) => (
									<tr
										key={`${group.id}-${record.type}-${record.name}-${record.value}`}
									>
										<td className="px-3 py-3 text-white">{record.type}</td>
										<td className="px-3 py-3 font-mono text-[11px] text-sky-100">
											{record.name}
										</td>
										...
										<td className="px-3 py-3">
											<span
												className={`inline-flex rounded px-2 py-1 ${statusClass(record.status || "pending")}`}
											>
	...
	return (
		<div className="max-w-5xl space-y-8">
			<div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
				<div>
					...
					<p className="mt-2 max-w-2xl text-sm text-text-muted">
						Connect a client domain, install the DNS records, then check
						verification before sending.
					</p>
				</div>
				{primaryDomain && (
					<div
						className={`inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-xs ${statusClass(primaryDomain.status)}`}
					>
			...
			<form
				...
				className="space-y-3 rounded-lg border border-glass-border bg-surface-1/70 p-4"
			>
				...
				<p className="text-xs text-text-muted">
					Use the root domain. DNS hosts usually want the name/value pairs
			...
			{domains.length === 0 ? (
				<div className="rounded-lg border border-dashed border-glass-border p-10 text-center">
					...
					<p className="mt-1 text-xs text-text-muted">
						Register a domain to generate SPF, DKIM, and DMARC instructions.
					</p>
				</div>
			) : (
				<div className="space-y-6">
					{domains.map((domain) => (
						<article
							key={domain.id}
							className="space-y-4 rounded-lg border border-glass-border bg-surface-1/70 p-5"
						>
							<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
								<div>
									...
									<p className="text-xs text-text-muted">
										Resend ID:{" "}
										<span className="font-mono">{domain.resendId}</span>
								...
								<div className="flex flex-wrap items-center gap-2">
									<span
										className={`inline-flex rounded-lg border px-3 py-2 text-xs ${statusClass(domain.status)}`}
									>
									...
									>
										<RefreshCcw
											size={14}
											className={pendingId === domain.id ? "animate-spin" : ""}
										/>
										Check Status
									</button>
								</div>
