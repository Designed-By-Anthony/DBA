...
import { complianceConfig, pipelineStages } from "@/lib/theme.config";
import type {
	Activity,
	ActivityType,
	ClickEvent,
...
function leadRowToProspect(row: LeadRow): Prospect {
	...
	const website =
		row.website || String(meta.website ?? meta.targetUrl ?? "").trim() || "";
	const notes =
		row.notes || String(meta.message ?? meta.biggest_issue ?? "").trim() || "";
	const auditUrl =
		typeof meta.auditUrl === "string" && meta.auditUrl.trim()
			? meta.auditUrl.trim()
			: null;
	const raw = row.status || "new";
	const status: ProspectStatus =
		raw === "new" ? "lead" : (raw as ProspectStatus);
	...
	return {
		...
		contractStatus:
			(row.contractStatus as Prospect["contractStatus"]) || undefined,
	...
	return withTenant(
		async (db, tenantId) => {
			const row = await db
				.select()
				.from(leads)
				.where(and(eq(leads.tenantId, tenantId), eq(leads.prospectId, id)))
				.limit(1);
...
> {
	return withTenant(async (db, tenantId) => {
		...
		const existing = await db
			...
			.from(leads)
			.where(
				and(eq(leads.tenantId, tenantId), eq(leads.emailNormalized, emailNorm)),
			)
			.limit(1);
		...
		if (existing.length > 0) {
			return {
				...
				duplicateOfId: existing[0].prospectId,
			};
		}
...
export async function updateProspect(
	...
	return withTenant(async (db, tenantId) => {
		...
		if (fields.targetUrl !== undefined)
			payload.targetUrl = fields.targetUrl || null;
		if (fields.dealValue !== undefined)
			payload.dealValue = Math.round(fields.dealValue * 100);
		...
		if (fields.assignedTo !== undefined)
			payload.assignedTo = fields.assignedTo || null;
		if (fields.status !== undefined) payload.status = fields.status;
		if (fields.lastContactedAt !== undefined)
			payload.lastContactedAt = fields.lastContactedAt || null;
		if (fields.unsubscribed !== undefined)
			payload.unsubscribed = fields.unsubscribed;
		if (fields.contractDocUrl !== undefined)
			payload.contractDocUrl = fields.contractDocUrl || null;
		if (fields.driveFolderUrl !== undefined)
			payload.driveFolderUrl = fields.driveFolderUrl || null;
		if (fields.contractSigned !== undefined)
			payload.contractSigned = fields.contractSigned;
		if (fields.contractStatus !== undefined)
			payload.contractStatus = fields.contractStatus || null;
		if (fields.stripeCustomerId !== undefined)
			payload.stripeCustomerId = fields.stripeCustomerId || null;
		...
		if (fields.pricingTier !== undefined)
			payload.pricingTier = fields.pricingTier || null;
		if (fields.projectNotes !== undefined)
			payload.projectNotes = fields.projectNotes || null;
		if (fields.fcmToken !== undefined)
			payload.fcmToken = fields.fcmToken || null;
...
export async function bulkUpdateStatus(
	...
	return withTenant(async (db, tenantId) => {
		await db
			...
			.where(and(eq(leads.tenantId, tenantId), inArray(leads.prospectId, ids)));
...
<body style="margin:0;padding:0;background:#f9fafb;font-family:'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:40px 24px;">
	...
	const result = await sendMail({
		...
		subject:
			`[TEST] ` +
			mergeTemplateVars(params.subject, {
				name: pName.replace(/[\r\n]+/g, " "),
			...
			try {
				...
				if (prospectId) {
					query = db
						...
						.from(emails)
						.where(
							and(eq(emails.tenantId, tenantId), eq(emails.leadId, prospectId)),
						)
						.orderBy(desc(emails.createdAt))
						.limit(100);
				}
		...
		async (db, tenantId) => {
			try {
				...
				const sentEmailResult = await db
					.select({ count: count() })
					.from(emails)
					.where(and(eq(emails.tenantId, tenantId), eq(emails.status, "sent")));

				const scheduledEmailResult = await db
					.select({ count: count() })
					.from(emails)
					.where(
						and(eq(emails.tenantId, tenantId), eq(emails.status, "scheduled")),
					);
				...
				prospectRows.forEach((p) => {
					...
					if (mappedStatus !== "launched") {
						...
						const days = Math.floor(
							(nowMs - createdAtMs) / (1000 * 60 * 60 * 24),
						);
				...
				return {
					...
					pipelineVelocityDays:
						launchedCount > 0
							? Math.round(totalVelocityDays / launchedCount)
							: 0,
				};
			} catch (err) {
...
export async function getCrmTasksForProspect(
	prospectId: string,
): Promise<CrmTask[]> {
	return withTenant(async (db, tenantId) => {
		...
		const rows = await db
			.select()
			.from(tasks)
			.where(and(eq(tasks.tenantId, tenantId), eq(tasks.leadId, prospectId)))
