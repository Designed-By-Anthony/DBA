...
export default function PriceBookPage() {
	...
	const loadData = useCallback(async () => {
		...
		try {
			...
			const msg =
				e instanceof Error ? e.message : "Failed to load Stripe catalog";
			setError(msg);
			setProducts([]);
		}
		setLoading(false);
	...
	const handleArchiveToggle = async (
		...
		setBusyId(productId);
		const res = await setStripeProductActiveAction({
			productId,
			active: nextActive,
		});
		if (!res.ok) {
	...
	const handleCreate = async (e: React.FormEvent) => {
		...
		if (res.error) {
			...
			toast.success(
				"Product published to Stripe — available for quotes & payment links.",
			);
			setShowModal(false);
			setForm({
				name: "",
			...
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
				<div>
					...
					<p className="text-sm text-text-muted mt-1 max-w-xl">
						Source of truth for services and retainers in Stripe. Products here
						power proposals, checkout, and payment links from prospect records —
						keep names and prices aligned with what clients see.
					</p>
				</div>
					...
					{products.length === 0 ? (
						<div className="col-span-full text-center py-12 px-4 rounded-xl border border-glass-border bg-glass-bg">
							...
							<p className="text-text-muted font-medium">
								Your price book is completely empty
							</p>
							<button
								onClick={() => setShowModal(true)}
						...
						products.map((p) => (
							<div
								key={p.id}
								className="rounded-xl border border-glass-border bg-glass-bg p-5 backdrop-blur-sm group relative overflow-hidden"
							>
								<div className="absolute top-0 right-0 p-3">
									<span
										className={`px-2 py-1 text-[10px] font-bold uppercase rounded ${p.active ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`}
									>
					...
					<div className="relative w-full max-w-md rounded-2xl border border-glass-border bg-[#0d0e14]/95 backdrop-blur-xl shadow-[0_0_80px_rgba(37,99,235,0.08),0_24px_48px_rgba(0,0,0,0.6)] animate-fade-in overflow-hidden">
						...
						<div className="flex items-center justify-between px-6 py-4 border-b border-glass-border bg-gradient-to-r from-surface-2/80 to-transparent rounded-t-2xl">
							<div>
								...
								<p className="text-[11px] text-text-muted mt-0.5">
									Publish a product to your Stripe catalog
								</p>
							</div>
							<button
								onClick={() => setShowModal(false)}
						...
						<form onSubmit={handleCreate} className="px-6 py-5 space-y-4">
							<div className="space-y-1.5">
								<label className="text-[10px] font-semibold uppercase tracking-wider text-text-gray">
									Product Name
								</label>
								<input
									autoFocus
