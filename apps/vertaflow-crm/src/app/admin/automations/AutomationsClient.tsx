...
export default function AutomationsClient({
	...
	const [draftTrigger, setDraftTrigger] = useState<AutomationTrigger>(
		"prospect_status_changed",
	);
	const [draftAction, setDraftAction] =
		useState<AutomationAction["type"]>("add_tag");
	...
	const handleSaveDraft = async () => {
		...
		try {
			const payload: Record<string, unknown> =
				draftAction === "add_tag" ? { tag: "System_Automated" } : {};
			const newId = await createAutomation(draftName, draftTrigger, {
				type: draftAction,
				payload,
			});
	...
	return (
		<div className="max-w-4xl mx-auto py-8">
			...
			<p className="text-text-muted text-sm mb-10 animate-fade-in">
				Zero-friction automation constraints. Build custom rules to let the CRM
				orchestrate itself.
			</p>
							...
							<div className="flex flex-col gap-5">
								...
								<div className="flex items-center gap-4 flex-wrap">
									<div className="flex items-center gap-2 text-sm text-text-muted">
										<span className="font-mono bg-surface-2 px-2 py-1 rounded text-xs">
											IF
										</span>
										<select
											value={draftTrigger}
											onChange={(e) =>
												setDraftTrigger(e.target.value as AutomationTrigger)
											}
								...
								<div className="flex justify-end gap-3 mt-4 border-t border-glass-border pt-4">
									<button
										...
										className="text-xs text-text-muted hover:text-white px-3 py-1.5 transition-colors"
									>
										Cancel
									</button>
									<button
					...
					<motion.div
						...
						className={`group flex items-center justify-between p-4 rounded-xl border transition-all ${
							rule.isActive
								...
								: "bg-(--color-surface-0) border-dashed border-glass-border opacity-60"
						}`}
					>
						<div className="flex items-center gap-4 min-w-0">
							...
							>
								<div
									className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${rule.isActive ? "translate-x-4" : "translate-x-0"}`}
								/>
							</button>
							<div className="min-w-0">
								<p className="text-sm font-semibold text-white truncate flex items-center gap-2">
									{rule.name}
									{!rule.isActive && (
										<span className="text-[10px] font-mono bg-surface-3 text-text-muted px-1.5 rounded uppercase">
											Paused
										</span>
									)}
								</p>
								<div className="flex items-center gap-2 text-xs text-text-muted mt-1 truncate">
									...
									<span className="font-mono text-white/50">IF</span>{" "}
									{TRIGGER_LABELS[rule.trigger]}
									<ArrowRight size={10} className="shrink-0 text-white/30" />
									<span className="font-mono text-white/50">THEN</span>{" "}
						...
						<div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
							<button className="p-2 text-text-muted hover:text-white rounded hover:bg-surface-2 transition-colors">
								<Settings2 size={16} />
							</button>
							<button
