					...
					{[
						{
							...
							value: `$${(d.weightedPipelineValue ?? d.forecastedMrr ?? 0).toLocaleString()}`,
						},
						{
							label: "Pipeline value",
							value: `$${d.pipelineValue.toLocaleString()}`,
						},
						{
							label: "Stale leads (14d+)",
							value: String(d.staleLeadCount ?? 0),
						},
						{
							label: "Overdue tasks",
							value: String(d.overdueOpenTasksCount ?? 0),
						},
					].map((x) => (
						<div
							key={x.label}
							className="glass-card p-4 rounded-xl border border-glass-border"
						>
					...
					<div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
						...
						<div>
							<p className="text-text-muted">
								Awaiting first response (in SLA window)
							</p>
							<p className="text-2xl font-bold text-amber-300">
								{t.awaitingFirstResponse}
							</p>
						</div>
					</div>
					<p className="text-xs text-text-muted mt-4">
						SLA uses wall-clock hours from ticket creation by priority (same
						rules as the Tickets page).
					</p>
				</div>
			)}
