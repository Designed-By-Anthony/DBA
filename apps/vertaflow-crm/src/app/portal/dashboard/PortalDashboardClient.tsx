...
> = {
	...
	contacted: {
		label: "In Discussion",
		color: "#0ea5e9",
		description: "We're discussing your project requirements.",
	},
	proposal: {
		label: "Proposal Sent",
		color: "#f59e0b",
		description:
			"Your proposal has been sent — review and sign to get started!",
	},
	dev: {
		label: "In Development",
		color: "#1d4ed8",
		description:
			"Your project is actively being built. Stay tuned for milestones!",
	},
	launched: {
		label: "Live!",
		color: "#10b981",
		description: "Your website is live and performing. Welcome aboard!",
	},
};
...
export default function PortalDashboard() {
	...
	const [activeTab, setActiveTab] = useState<"dashboard" | "viewer">(
		"dashboard",
	);
	const [viewerDevice, setViewerDevice] = useState<
	...
	if (!data) {
		return (
			<div className="text-center py-20">
				...
				<a
					href="/portal"
					className="text-(--color-brand) hover:underline text-sm"
				>
					Log in again →
				</a>
			</div>
					...
					<div className="flex-1 bg-black/50 flex items-center justify-center p-4 overflow-hidden relative">
						<div
							className={`transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] bg-white shadow-2xl relative ${
								viewerDevice === "mobile"
									? "w-[375px] h-[812px] rounded-4xl border-8 border-neutral-800"
									: viewerDevice === "tablet"
										? "w-[768px] h-[1024px] rounded-2xl border-8 border-neutral-800"
								...
								<div className="space-y-3">
									{onboardingItems.map((item, i) => (
										<div key={i} className="flex items-center gap-3">
											...
											<span
												className={`text-sm ${item.done ? "text-white/60 line-through" : "text-white"}`}
											>
												{item.label}
											</span>
										</div>
