...
export default function QuoteClientInteractive({
	quote,
	companyName,
}: {
	quote: Quote;
	...
	const [selectedPackage, setSelectedPackage] = useState<QuotePackage | null>(
		quote.packages.length === 1 ? quote.packages[0] : null,
	);
	...
	return (
		<div className="space-y-8 pb-20">
			...
			<div className="space-y-4">
				...
				<div
					className={`grid gap-6 ${quote.packages.length === 1 ? "grid-cols-1 max-w-2xl mx-auto" : "grid-cols-1 md:grid-cols-3"}`}
				>
						...
						>
							...
							<div className="my-6 space-y-1">
								...
								<p className="text-3xl font-bold text-white font-mono">
									{formatCents(pkg.totalOneTimeCents)}
								</p>
								...
								{pkg.totalRecurringCents > 0 && (
									<div className="pt-2">
										...
										<p className="text-xl font-semibold text-[var(--color-brand)] font-mono">
											{formatCents(pkg.totalRecurringCents)} / mo
										</p>
									</div>
								)}
							</div>
							...
							<ul className="space-y-3 pt-6 border-t border-[var(--color-glass-border)]">
								{pkg.items.map((item, idx) => (
									<li
										key={idx}
										className="flex items-start gap-2 text-sm text-[var(--color-text-gray)]"
									>
			...
			{quote.status === "accepted" ? (
				...
				<div className="bg-[var(--color-surface-1)] border border-[var(--color-glass-border)] rounded-2xl p-6 md:p-8">
					...
					<div className="prose prose-invert prose-sm max-w-none text-[var(--color-text-gray)] mb-6">
						<p>
							By signing below, {companyName} agrees to authorize the selected
							digital marketing packages and initiates the billing agreement for
							the initial down payment and any subsequent recurring
							software/service retainers. The terms outined in the Master
