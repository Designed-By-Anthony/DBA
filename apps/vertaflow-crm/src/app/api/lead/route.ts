import type {
	...
	PublicLeadMarketingMeta,
} from "@dba/lead-form-contract";
...
function marketingMetaFromPublicBody(
	b: PublicLeadIngestBody,
): PublicLeadMarketingMeta | undefined {
	...
	return NextResponse.json(
		{
			...
			honeypot:
				"Include _hp as empty string (hidden field) to reduce spam bots.",
...
export async function POST(request: NextRequest) {
	...
	if (!turnstileEnforced) {
		...
		const bypassOk =
			process.env.PUBLIC_LEAD_INGEST_ALLOW_NO_TURNSTILE === "true";
		if (isProd && !bypassOk) {
			return NextResponse.json(
	...
	try {
		const parsedBody = await readBoundedJson<unknown>(
			request,
			LEAD_INGEST_MAX_BYTES,
		);
		if (!parsedBody.ok) {
		...
		if (turnstileEnforced) {
			const tv = await verifyTurnstileToken(
