...
export async function PATCH(
	...
	try {
		...
		const { adminReply, status } = (await request.json()) as {
			adminReply?: string;
		...
		const existingMessages = (ticket.messages || []) as Array<{
			from?: string;
		}>;
		...
		if (ticket.leadEmail && hasReply) {
			...
			const safeAddress = complianceConfig.physicalAddress.replace(
				/\\n/g,
				", ",
			);
			try {
