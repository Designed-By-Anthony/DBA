...
const signContractSchema = z.object({
	...
	consentGiven: z.literal(true, {
		message: "Electronic signature consent is required",
	}),
});
...
export async function POST(req: NextRequest) {
	try {
		...
		const result = await withTenantContext(db, orgId, async (tx) => {
			...
			const nextNum =
				existing.length > 0
					? parseInt(existing[0].contractNumber.replace(/\D/g, "") || "0", 10) +
						1
					: 1;
...
export async function PATCH(req: NextRequest) {
	try {
		...
		if (!body.success) {
			return NextResponse.json(
				{ error: "Invalid signature payload", details: body.error.flatten() },
				{ status: 400 },
			);
		}
