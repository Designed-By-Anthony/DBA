import { activities, getDb, leads, withBypassRls } from "@dba/database";
...
export async function POST(request: NextRequest) {
	...
	if (!signingKey) {
		console.error(
			"[calendly] webhook rejected: CALENDLY_WEBHOOK_SIGNING_KEY not configured",
		);
	...
	try {
		...
		if (!defaultAgencyId) {
			...
			return NextResponse.json(
				{
					error:
						"Server misconfigured: set LEAD_WEBHOOK_DEFAULT_AGENCY_ID to your Clerk org id",
				},
				{ status: 503 },
			);
		}
