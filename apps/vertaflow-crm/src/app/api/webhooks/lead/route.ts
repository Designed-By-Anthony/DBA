...
export async function GET(request: NextRequest) {
	...
	const secretOk =
		expected.length > 0 && timingSafeEqualStr(provided, expected);
...
export async function POST(request: NextRequest) {
	...
	try {
		const parsed = await readBoundedJson<LeadWebhookBody>(
			request,
			LEAD_WEBHOOK_MAX_BYTES,
		);
		if (!parsed.ok) {
			...
			return NextResponse.json(
				{
					...
					secretOk: true,
					message:
						"Webhook secret and agency resolution OK. No prospect created.",
				},
				{ headers: cors },
			);
		}
		...
		const message = String(
			body.message || body.projectRequirements || "",
		).trim();
