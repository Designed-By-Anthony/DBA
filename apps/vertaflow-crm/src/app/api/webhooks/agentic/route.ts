import { activities, getDb, leads } from "@dba/database";
import { eq } from "drizzle-orm";
import { NextResponse } from "next/server";
import {
	evaluateProspectHealth,
...
export async function POST(req: Request) {
	...
	if (!secret) {
		console.error(
			"[agentic] webhook rejected: AGENTIC_WEBHOOK_SECRET not configured",
		);
	...
	try {
		...
		if (
			...
			return NextResponse.json(
				{ error: "Missing required Agentic context parameters" },
				{ status: 400 },
			);
		}
		...
		if (
			msg.includes("ready to start") ||
			msg.includes("send the contract") ||
			msg.includes("let's do it")
		) {
		...
		return NextResponse.json(
			{ error: "Failed to process agentic evaluation" },
			{ status: 500 },
		);
	}
}
