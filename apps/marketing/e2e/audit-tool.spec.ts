...
test.describe("Audit Tool", () => {
	...
	test("invisible Turnstile widget is attached (no visible box)", async ({
		page,
	}) => {
		await page.goto("/free-seo-audit");
	...
	test("Turnstile async errors restore the audit submit state", async ({
		page,
	}) => {
		...
		await expect(btn).toHaveText("Run Free Audit");
		await expect(page.locator("[data-lh-error]")).toContainText(
