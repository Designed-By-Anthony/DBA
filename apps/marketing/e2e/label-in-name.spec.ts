...
test('report target link accessible name starts with the visible URL', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    inlineReport.setReportData({
      ...
      diagnostics: {},
      aiInsight: {
        summary: 'Strong local service-business homepage with clear calls to action.',
  ...
  const visibleText = ((await targetLink.textContent()) || '').replace(/\s+/g, ' ').trim();
