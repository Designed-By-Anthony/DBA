...
async function getBoundaryContrast(page: Page, selector: string) {
  return page.evaluate((target: string) => {
    ...
    const background =
      ...
      parseColor(getComputedStyle(document.body).backgroundColor) ?? { r: 15, g: 18, b: 24, a: 1 };

    return contrast(
      blend(border, background),
      { r: background.r, g: background.g, b: background.b },
    );
  }, selector);
}

test('reach-out sticky and modal controls meet non-text contrast minimum', async ({ page }) => {
  await page.goto('/', { waitUntil: 'domcontentloaded' });
  await dismissCookieConsentIfPresent(page);

  await expect(getBoundaryContrast(page, '.reach-out-sticky-btn')).resolves.toBeGreaterThanOrEqual(3);

  await page.locator('#reachOutOpenBtn').click();
  await expect(getBoundaryContrast(page, '.reach-out-action-icon')).resolves.toBeGreaterThanOrEqual(3);
});

test('report toolbar button boundaries meet non-text contrast minimum', async ({ page }) => {
  await page.goto('/report?id=DBA-DESI6HSF', { waitUntil: 'domcontentloaded' });
  await expect(getBoundaryContrast(page, '.toolbar-btn')).resolves.toBeGreaterThanOrEqual(3);
});
