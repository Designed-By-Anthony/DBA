...
test.describe("Mobile UX", () => {
	...
	test("sticky bar opens reach-out modal with contact, call, email, and Calendly", async ({
		page,
	}) => {
		await page.goto("/");
		await dismissCookieConsentIfPresent(page);
		...
		await expect(
			dialog.locator('a[href*="mailto:anthony@designedbyanthony.com"]'),
		).toBeVisible();
		...
		await expect(calModal.locator('iframe[src*="calendly.com"]')).toBeVisible({
			timeout: 20_000,
		});
	});
