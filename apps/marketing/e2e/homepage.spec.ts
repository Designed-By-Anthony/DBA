...
test.describe('Homepage', () => {
  ...
  test('founding partner section is visible with clear CTAs', async ({ page }) => {
    await page.goto('/', { waitUntil: 'domcontentloaded' });
    const foundingSection = page.locator('.founding-partner-shell');
    await expect(foundingSection).toBeVisible();
    await expect(foundingSection.locator('a[href*="calendly.com"]').first()).toBeVisible();
    await expect(foundingSection.locator('a[href="/free-seo-audit"]').first()).toBeVisible();
  });
