...
const coreServiceCatalog = [
  {
    ...
    description: 'Custom websites built for trust, speed, clarity, and lead generation.',
  },
  {
    name: 'Website Rescues and Mobile Optimization',
    path: '/services/website-rescue',
    description: 'Website rescue work that improves mobile usability, speed, and conversion flow.',
  },
  {
    name: 'Managed Hosting and VIP Support',
    path: '/services/managed-hosting',
    description: 'Managed hosting and direct support that keep the site fast, secure, and easy to maintain.',
  },
  {
...
export function buildBreadcrumbSchema(pathname: string, items: BreadcrumbItem[]): SchemaValue | null {
  if (items.length < 2) {
    return null;
  }
...
export function buildServiceAreaSchema(input: ServiceAreaSchemaInput): SchemaValue[] {
  ...
  const serviceSchema: SchemaValue = {
    ...
    name: input.name,
    description: input.description,
    serviceType: input.serviceType ?? 'Custom web design, website rescue, managed hosting, and local SEO',
