		...
		try {
			...
			try {
				...
				if (
					...
					logger.warn(
						"Doc: https://www.bing.com/indexnow/getstarted · https://www.bing.com/webmasters",
					);
				}
			} catch {
...
export default function indexNowStaticPages(options = {}) {
	...
	const fallbackEndpoints =
		options.fallbackEndpoints !== undefined
	...
	return {
		...
		hooks: {
			...
			"astro:build:done": async ({ dir, logger }) => {
				...
				if (anyBatchFailed) {
					logger.warn(
						"finished with failures — URLs may retry on the next build if hashes change again",
					);
				} else {
