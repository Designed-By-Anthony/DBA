...
async function postSyncBatch(records: OfflineRecord[]): Promise<SyncBatchResponse> {
...
async function markSynced(localId: string): Promise<void> {
  ...
  if (estimate) {
    const nextEstimate: LocalEstimateRecord = { ...estimate, sync_status: "synced" };
    await localDb.estimates.put(nextEstimate);
  }
}

export async function syncPendingRecords(syncApi: SyncApi = postSyncBatch): Promise<SyncBatchResponse> {
  if (typeof navigator !== "undefined" && !navigator.onLine) {
    return { syncedLocalIds: [] };
  }

  const pendingLeads = await localDb.leads.where("sync_status").equals("pending").toArray();
  const pendingEstimates = await localDb.estimates.where("sync_status").equals("pending").toArray();
  ...
  if (result.syncedLocalIds.length > 0) {
    await localDb.transaction("rw", localDb.leads, localDb.estimates, async () => {
      await Promise.all(result.syncedLocalIds.map((localId) => markSynced(localId)));
    });
  }
