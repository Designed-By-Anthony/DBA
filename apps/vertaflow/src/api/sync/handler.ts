...
import { estimates, getDb, leads, withTenantContext, type Database } from "@dba/database";
import {
...
function isIncomingMoreRecent(existingUpdatedAt: string, incomingUpdatedAt: string): boolean {
  return new Date(incomingUpdatedAt).getTime() > new Date(existingUpdatedAt).getTime();
}

async function upsertLeadRecord(tx: Database, tenantId: string, lead: LeadSyncRecord): Promise<boolean> {
...
async function upsertEstimateRecord(
  ...
  const existing = await tx
    .select()
    .from(estimates)
    .where(and(eq(estimates.tenantId, tenantId), eq(estimates.id, estimate.server_id)))
    .limit(1);
...
export async function syncOfflineRecords(
  ...
  if (!db) {
    return {
      ...
      estimates: { accepted: 0, ignored: parsed.estimates.length, source: "noop" },
    };
  }
  ...
  await withTenantContext(db, parsed.tenant_id, async (tx) => {
    for (const lead of parsed.leads) {
      const didWrite = await upsertLeadRecord(tx as unknown as Database, parsed.tenant_id, lead);
      if (didWrite) {
