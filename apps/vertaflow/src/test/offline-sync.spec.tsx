...
describe("offline-first synchronization", () => {
  ...
  it("queues offline lead and syncs when connectivity returns", async () => {
    ...
    const headerValue = calledInit?.headers instanceof Headers
      ? calledInit.headers.get("Content-Type")
      : (calledInit?.headers as Record<string, string> | undefined)?.["Content-Type"];
    ...
    expect(postedBody.records?.[0]?.local_id).toBe("11111111-1111-4111-8111-111111111111");
    expect(postedBody.records?.[0]?.sync_status).toBe("pending");

    const pendingLead = await localDb.leads.get("11111111-1111-4111-8111-111111111111");
