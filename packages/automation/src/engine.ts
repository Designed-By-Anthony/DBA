...
import { getDb, automations } from "@dba/database";
...
export async function loadActiveRules(
  ...
  for (const r of rows) {
    const parsed = automationRuleSchema.safeParse({
      ...
      action: r.action,
      verticalScope: (r.metadata as { verticalScope?: VerticalId })?.verticalScope,
    });
    if (parsed.success) {
      out.push(parsed.data);
    } else {
      console.warn(`[automation] dropping invalid rule ${r.id}`, parsed.error.flatten());
    }
  }
  return out;
}

function mergeRules(sqlRules: AutomationRule[], defaults: AutomationRule[]): AutomationRule[] {
 ...
 */
export async function runAutomations(opts: RunAutomationsOptions): Promise<AutomationRunOutcome[]> {
  ...
  for (const rule of rules) {
    try {
      ...
      if (!passes) {
        outcomes.push({ ruleId: rule.id, ruleName: rule.name, skipped: "condition_not_met" });
        continue;
      }
      ...
      if (!handler) {
        outcomes.push({ ruleId: rule.id, ruleName: rule.name, skipped: "handler_missing" });
        continue;
      }

      // `handler` is the union — we intentionally narrow via the discriminant.
      const result = await (handler as (a: AutomationAction, c: ActionHandlerContext) => Promise<
        { ok: true; detail?: string } | { ok: false; error: string }
      >)(parsedAction, { rule, event });

      if (result.ok) {
        outcomes.push({ ruleId: rule.id, ruleName: rule.name, ok: true, detail: result.detail });
      } else {
        outcomes.push({ ruleId: rule.id, ruleName: rule.name, ok: false, error: result.error });
      }
    } catch (err) {
      const msg = err instanceof Error ? err.message : "unknown_error";
      console.error(`[automation] rule ${rule.id} (${rule.name}) crashed:`, err);
      outcomes.push({ ruleId: rule.id, ruleName: rule.name, ok: false, error: msg });
    }
  }
