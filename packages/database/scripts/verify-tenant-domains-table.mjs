...
if (!url?.trim()) {
	console.warn(
		"SKIP: No DATABASE_URL — set in env to verify tenant_domains exists on Neon.",
	);
	process.exit(0);
}
...
try {
	...
	console.error(
		"FAIL: public.tenant_domains not found — apply packages/database/drizzle/0002_tenant_email_domains.sql (or sql/2026-04-18-tenant-email-domains.sql).",
	);
	process.exit(1);
} catch (e) {
