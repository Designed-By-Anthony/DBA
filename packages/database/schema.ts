...
export const tenants = pgTable("tenants", {
	...
	dealSources: jsonb("deal_sources").$type<string[]>().notNull().default([]),
...
export const tenantDomains = pgTable(
	...
	(table) => [
		uniqueIndex("uniq_tenant_domains_tenant_domain").on(
			table.tenantId,
			table.domainName,
		),
		uniqueIndex("uniq_tenant_domains_tenant_resend").on(
			table.tenantId,
			table.resendId,
		),
...
export const notifications = pgTable(
	...
	(table) => [
		index("idx_notifications_tenant_id_is_read").on(
			table.tenantId,
			table.isRead,
		),
	],
...
export const pushSubscriptions = pgTable(
	...
	(table) => [
		index("idx_push_subscriptions_tenant_user").on(
			table.tenantId,
			table.userId,
		),
	],
...
export type AutomationActionType =
	(typeof automationActionTypeEnum.enumValues)[number];
export type NotificationChannel =
	(typeof notificationChannelEnum.enumValues)[number];
export type StripeConnectStatus =
	(typeof stripeConnectStatusEnum.enumValues)[number];
...
export type EventBookingStatus =
	(typeof eventBookingStatusEnum.enumValues)[number];
