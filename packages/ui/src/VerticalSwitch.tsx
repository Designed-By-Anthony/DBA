...
import { z } from "zod";
import { VERTICAL_IDS, type VerticalId, getVerticalConfig } from "./vertical-config";
...
function resolveVertical(input: VerticalId | string | null | undefined): VerticalId {
  const parsed = z.enum(VERTICAL_IDS).safeParse(input);
  return parsed.success ? parsed.data : "agency";
}

export function VerticalSwitch(props: VerticalSwitchProps): React.ReactElement | null {
 ...
 */
export function useActiveVertical(vertical: VerticalId | string | null | undefined) {
