				...
				Array.isArray(v) ? (
					<div className="flex flex-wrap gap-1">
						{v.map((t) => (
							<span
								key={String(t)}
								className="rounded bg-white/10 px-1 text-[10px] text-white/80"
							>
								{String(t)}
							</span>
						))}
...
export function GenericLeadCard({ lead, vertical, actionSlot }: LeadCardProps) {
	const verticalId: VerticalId = VERTICAL_IDS_SET.has(vertical as VerticalId)
...
// ─── helpers ─────────────────────────────────────────────────────────────────
const VERTICAL_IDS_SET = new Set<VerticalId>([
	"agency",
	"service_pro",
	"restaurant",
	"florist",
...
function formatScore(v: unknown): React.ReactNode {
	...
	return (
		<span>
			{v}
			<span className="text-white/40 text-[10px] ml-0.5">/ 100</span>
		</span>
	);
}
