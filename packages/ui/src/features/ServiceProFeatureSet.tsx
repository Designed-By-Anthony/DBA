...
import type { ServiceProLeadMetadata, ServiceProJobStatus } from "../vertical-metadata";
...
export function ServiceProFeatureSet({ leads, renderLeadCard }: ServiceProFeatureSetProps) {
  return (
    <section
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] text-white/50">{colLeads.length}</span>
            </div>
            <ul className="space-y-2">
              {colLeads.map((lead) => (
                ...
                >
                  ...
                  <div className="text-[11px] text-white/60 truncate">{lead.email}</div>
                  {lead.metadata?.geo ? (
                    <div className="text-[10px] text-white/50 mt-1">
                      📍 {lead.metadata.geo.lat.toFixed(3)}, {lead.metadata.geo.lng.toFixed(3)}
                    </div>
                  ) : null}
                  {lead.metadata?.addressLine ? (
                    <div className="text-[10px] text-white/50 truncate">{lead.metadata.addressLine}</div>
                  ) : null}
