...
export function AgencyFeatureSet({
	metadata,
	renderAuditViewer,
}: AgencyFeatureSetProps) {
	const m = metadata ?? {};
	const auditUrl = m.lighthouseReportUrl;
	return (
		<section
			aria-label="Agency feature set"
			data-vertical="agency"
			className="space-y-4"
		>
	...
	return (
		<div className="rounded-lg border border-white/10 bg-white/5 p-3">
			...
			<p className="text-xl font-semibold text-white mt-1">
				{value ?? "—"}
				{value != null && suffix ? (
					<span className="text-xs text-white/50 ml-1">{suffix}</span>
				) : null}
			</p>
		</div>
	);
