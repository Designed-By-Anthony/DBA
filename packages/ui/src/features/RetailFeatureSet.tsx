...
export function RetailFeatureSet({
	...
	const stale = leads.filter((l) => {
		const last = l.metadata?.lastPurchaseAt
			? Date.parse(l.metadata.lastPurchaseAt)
			: NaN;
		...
		>
			<div className="grid grid-cols-2 md:grid-cols-3 gap-3">
				...
				<Kpi
					...
					value={stale.length}
				/>
				<Kpi label="Avg CLV" value={avgClv(leads)} prefix="$" />
			</div>
			<div
				data-slot="re-engagement"
			...
			>
				...
				{stale.length === 0 ? (
					<p className="text-xs text-white/50">
						Nobody is stale yet. Keep shipping.
					</p>
				) : (
